...
def _career_projection(
    ...
    *,
    ...
    scored.sort(key=lambda item: item[1], reverse=metric not in LOWER_IS_BETTER)
    peak_row, peak_value = scored[0]
    if mode == "peak":
        return dict(peak_row), peak_value, dict(peak_row)
    selected = (
        [dict(item[0]) for item in scored[: max(1, best_n)]]
        if mode == "best_n"
        else [dict(item[0]) for item in scored]
    )
    ...
    with _connect() as db:
        ...
        coverage = _rows(
            db.execute(
                ...
                SELECT domain,league_id,MIN(season_id) AS first_season,MAX(season_id) AS last_season,
                       COUNT(DISTINCT season_id) AS seasons,SUM(row_count) AS rows,
        ...
        counts = {
            "field_provenance": int(
                db.execute("SELECT COUNT(*) FROM canon_field_provenance").fetchone()[0]
            ),
            "material_conflicts": int(
                db.execute("SELECT COUNT(*) FROM canon_conflicts").fetchone()[0]
            ),
            ...
            "franchises": int(
                db.execute("SELECT COUNT(*) FROM canon_dim_franchise").fetchone()[0]
            ),
            "games": int(db.execute("SELECT COUNT(*) FROM canon_dim_game").fetchone()[0]),
            "player_games": int(
                db.execute("SELECT COUNT(*) FROM canon_fact_player_game").fetchone()[0]
            ),
        }
        return {
            ...
            "sources": sources,
            "coverage": coverage,
            "top_conflict_fields": conflict_fields,
        }
...
def historical_player_games(
    ...
    with _connect() as db:
        ...
        items = _rows(db.execute(sql, [*params, limit, offset]).fetchall())
        for item in items:
...
def historical_all_time(
    ...
    collapsed = _collapse_across_seasons(
        raw, combine_segments=season_type == "combined"
    )
    ...
    for player_key, player_rows in by_player.items():
        ...
        career_games = sum(_num(row, "games") or 0 for row in player_rows)
        if season_count < min_seasons or career_games < min_games:
            continue
        aggregate, metric_value, peak = _career_projection(
            player_rows,
            metric=metric,
            basis=basis,
            mode=mode,
        ...
        if aggregate is None or peak is None or metric_value is None:
            continue
        seasons = sorted(
            {str(row.get("season_id") or "") for row in player_rows}, key=_season_sort
        )
        peak_metric, _ = _scaled(peak, metric, basis)
        output.append(
            {
                "player_key": player_key,
...
def historical_compare(
    ...
    season_type = _season_type(season_type)
    keys = list(
        dict.fromkeys(key.strip() for key in player_keys.split(",") if key.strip())
    )
    if not 2 <= len(keys) <= 6:
    ...
    with _connect() as db:
        ...
        identities = {
            ...
            for row in db.execute(
                f"SELECT * FROM canon_dim_player WHERE player_key IN ({placeholders})", keys
            ).fetchall()
        }
...
def historical_franchise(franchise_key: str) -> dict[str, Any]:
    with _connect() as db:
        ...
        teams = _rows(
            db.execute(
                "SELECT * FROM canon_dim_team WHERE franchise_key=? ORDER BY active_from,canonical_name",
                (franchise_key,),
            ).fetchall()
        )
        ...
        return {"franchise": dict(franchise), "teams": teams, "seasons": seasons}
