...
def insert_player_season(
    ...
    *,
    ...
    columns = (
        "fact_key,player_key,season_id,league_id,season_type,team_key,team_abbreviation,position,age,"
        "games,games_started,minutes,fgm,fga,fg_pct,three_pm,three_pa,three_pct,ftm,fta,ft_pct,"
        "reb,ast,pts,ts_pct,efg_pct,ws,bpm,primary_source,source_count,provenance_json"
    )
    values = (
        fact_key,
        player_key,
    ...
    db.execute(
        f"INSERT INTO canon_fact_player_season({columns}) VALUES ({','.join('?' for _ in values)})",
        values,
    )
    ...
    try:
        db.executescript(
            ...
            CREATE TABLE canon_fact_award(
              ...
              award TEXT,rank_text TEXT,winner INTEGER,share REAL,source_key TEXT,payload_json TEXT
            );
            CREATE TABLE canon_fact_all_star(
              ...
              team_text TEXT,source_key TEXT,payload_json TEXT
            );
            CREATE TABLE canon_fact_draft(
              ...
              source_key TEXT,payload_json TEXT
            );

            CREATE TABLE canon_field_provenance(
              entity_type TEXT,entity_key TEXT,field_name TEXT,source_key TEXT,source_table TEXT,
              source_row TEXT,source_value TEXT,selected INTEGER,evidence_json TEXT
            );
            ...
            CREATE TABLE canon_conflicts(
              ...
              selected_source TEXT,selected_value TEXT,alternate_source TEXT,alternate_value TEXT,
            ...
            INSERT INTO canon_conflicts(
              ...
              ('player_season','star24','pts','sumitro_bref_history','2400',
               'gonzalo_all_time','2300','material','now');

            CREATE TABLE canon_coverage(
              domain TEXT,league_id TEXT,season_id TEXT,row_count INTEGER,
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="sports-terminal-history-research-") as temp:
        ...
        assert games["matched_rows"] == 1, games
        assert games["rows"][0]["pts"] == 32.0, games
