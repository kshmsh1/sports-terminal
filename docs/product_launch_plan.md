...
| Local persistence | Expanded first version | The product has a shared local persistence service for theme, workbook cells, selected workbook sheet, NBA mode, last-opened NBA player/team/game, favorite teams, player watchlist, fantasy query, community board, liked posts, and profile settings. |
...
| Backend API | Durable local prototype | `backend/app/main.py` now exposes FastAPI endpoints backed by local SQLite for users, profiles, settings, personalization, workspaces, community, moderation, messaging, CMS, billing, feature flags, data sources, and pipeline runs. |
| Backend dev tooling | First version | `backend/scripts/dev.sh` uses `python3`, creates the virtualenv, installs dependencies, and launches Uvicorn. |
| Flutter API client | First scaffold | `lib/services/product_api_client.dart` defines a typed-ish HTTP client for health/readiness, users, profile/settings, personalization, workbooks, community, reports, billing plans, and feature flags. |
...
| Backend | API and data control plane | FastAPI + SQLite local-durable prototype with core product endpoints and readiness reporting. |
...
| Flutter/backend wiring | `ProductApiClient` exists, but app screens still need to sync to backend while keeping local offline fallback. |
...
| Community backend | SQLite-backed posts/comments/reactions/reports exist, but UI wiring, moderation queues, bans, mutes, blocks, follows, saves, and audit events remain. |
| Messaging backend | SQLite-backed conversations/messages exist, but UI wiring, unread counts, blocking, reporting, and notification delivery remain. |
| Fantasy features | Backend-synced league imports, roster management, scoring settings, waiver boards, matchup tools, and alerts. |
| CMS | Article endpoints exist, but admin editor UI, revisions, publish workflow, featured cards, tags, author pages, moderation, and audit history remain. |
...
1. Validate Flutter and backend after the SQLite/API-client pass.
2. Add auth/session scaffolding and user identity flow.
3. Wire Flutter profile, favorites, watchlists, community, messages, workspaces, and admin controls to `ProductApiClient` with local storage fallback.
...
| Launchable surface, expandable core | Prototype with local persistence and local backend now, then replace local state with hosted backend persistence once auth/deployment are ready. |
