...
class ProductAnalyticsSuiteScreen extends StatefulWidget {
  ...
  @override
  State<ProductAnalyticsSuiteScreen> createState() =>
      _ProductAnalyticsSuiteScreenState();
}

class _ProductAnalyticsSuiteScreenState
    extends State<ProductAnalyticsSuiteScreen> {
    ...
    return FutureBuilder<NbaTerminalSeedSnapshot>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState != ConnectionState.done) {
          return const _SuiteSurface(
            child: Text(
              'Loading Analytics Suite…',
              style: TextStyle(color: _suiteMuted),
            ),
          );
        }
        if (snapshot.hasError || snapshot.data == null) {
          return _SuiteSurface(
            child: Text(
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              _FeatureGrid(
                active: tool,
                onSelect: (value) => setState(() => tool = value),
              ),
              const SizedBox(height: 12),
      ...
      children: [
        const Column(
          ...
          children: [
            ...
            SizedBox(height: 3),
            Text(
              'One connected research surface, not twenty disconnected microsites.',
              style: TextStyle(
                color: _suiteText,
                fontSize: 19,
                fontWeight: FontWeight.w900,
...
const _features = <_FeatureDefinition>[
  _FeatureDefinition(
    'Player Dashboard',
    'Percentiles, recent form, shooting and impact profile.',
    Icons.person_search_rounded,
  ),
  _FeatureDefinition(
    'Player Compare',
    'Side-by-side metric and percentile comparison.',
    Icons.compare_arrows_rounded,
  ),
  _FeatureDefinition(
    'Team Compare',
    'Records, roster production and team context.',
    Icons.balance_rounded,
  ),
  _FeatureDefinition(
    'Rankings',
    'Rank every player by any supported metric.',
    Icons.format_list_numbered_rounded,
  ),
  _FeatureDefinition(
    'Last X Games',
    'Recent game logs and rolling form.',
    Icons.timeline_rounded,
  ),
  _FeatureDefinition(
    'Shot Profile',
    'Two, three and free-throw volume and efficiency.',
    Icons.radar_rounded,
  ),
  _FeatureDefinition(
    'Lineup Builder',
    'Build five-player groups from sourced roster production.',
    Icons.groups_rounded,
  ),
  _FeatureDefinition(
    'Tier List',
    'Transparent metric-driven player tiers.',
    Icons.emoji_events_rounded,
  ),
  _FeatureDefinition(
    'ORtg Sandbox',
    'Interactive possession-efficiency calculator.',
    Icons.calculate_rounded,
  ),
  _FeatureDefinition(
    'Data Coverage',
    'Inspect what is sourced, derived, estimated or unavailable.',
    Icons.fact_check_rounded,
  ),
  _FeatureDefinition(
    'WOWY Lineups',
    'Together/apart possession analysis.',
    Icons.hub_rounded,
    sourceRequired: true,
  ),
  _FeatureDefinition(
    'Matchup Matrix',
    'Tracking-based defender and matchup outcomes.',
    Icons.grid_view_rounded,
    sourceRequired: true,
  ),
  _FeatureDefinition(
    'Impact Decomposition',
    'RAPM and component-model research.',
    Icons.science_rounded,
    sourceRequired: true,
  ),
  _FeatureDefinition(
    'Shot Quality',
    'Pre-shot quality, making and contest decomposition.',
    Icons.track_changes_rounded,
    sourceRequired: true,
  ),
  _FeatureDefinition(
    'Draft Analysis',
    'Prospect rankings and historical comparisons.',
...
class _PlayerDashboard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final logs = data.playerGameLogsTop
        .where(
          (row) => _rawText(row, const ['player_id', 'id']) == player?.playerId,
        )
        .take(lastGames)
        .toList();
    final metrics = const [
    ...
    return _ToolSurface(
      ...
      subtitle:
          'A source-aware profile with visible percentiles and recent-game evidence.',
      controls: [
                  ...
                  items: [
                    for (final key in metrics.take(4))
                      _MetricCard(
                        ...
                        engine.formatValue(key, player.value(key)),
...
class _PlayerCompare extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return _ToolSurface(
      ...
      subtitle:
          'Compare values and percentile position without switching pages.',
      controls: [
        _PlayerDrop(
          rows: rows,
          value: playerA,
...
class _TeamCompare extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final rosterA = rows
        .where((row) => row.team.split(RegExp(r'[,/ ]+')).contains(teamA))
        .toList();
    final rosterB = rows
        .where((row) => row.team.split(RegExp(r'[,/ ]+')).contains(teamB))
        .toList();
    ...
    final metrics = <_TeamMetric>[
      _TeamMetric(
        'Wins',
        _recordValue(recordA, const ['wins', 'w']),
        _recordValue(recordB, const ['wins', 'w']),
      ),
      _TeamMetric(
        'Losses',
        _recordValue(recordA, const ['losses', 'l']),
        _recordValue(recordB, const ['losses', 'l']),
        lowerBetter: true,
      ),
      ...
      _TeamMetric(
        'Top-8 production',
        _topSum(rosterA, 'game_score_proxy', 8),
        _topSum(rosterB, 'game_score_proxy', 8),
      ),
      _TeamMetric(
        'Top-8 defensive events',
        _topSum(rosterA, 'defense_events', 8),
        _topSum(rosterB, 'defense_events', 8),
      ),
    ];
    return _ToolSurface(
      title: 'Team Compare',
      subtitle:
          'Team record context plus roster-production comparisons from the same normalized player layer.',
      controls: [
        _SmallDrop<String>(
          value: teamA,
          values: teams,
      ...
      child: Column(
        children: [
          for (final metric in metrics)
            _TeamComparisonRow(metric: metric, teamA: teamA, teamB: teamB),
          const SizedBox(height: 10),
          Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Expanded(
                child: _RosterList(team: teamA, rows: rosterA),
              ),
              const SizedBox(width: 10),
              Expanded(
                child: _RosterList(team: teamB, rows: rosterB),
              ),
            ],
          ),
        ],
...
class _RankingsTool extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final ranked = rows
        .where((row) => position == 'All' || row.position == position)
        .toList();
    engine.sortRows(ranked, metricKey);
    return _ToolSurface(
      title: 'Rankings',
      subtitle:
          'A transparent ranking table for every metric supported by the Stats Workstation.',
      controls: [
        ...
        children: [
          for (var index = 0; index < math.min(50, ranked.length); index++)
            _RankingRow(
              index: index + 1,
              row: ranked[index],
              value: engine.formatValue(
                metricKey,
                ranked[index].value(metricKey),
              ),
              percentile: ranked[index].percentiles[metricKey],
            ),
        ],
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    final points = [
      for (final row in logs.reversed)
        _rawNumber(row, const ['pts', 'points']) ?? 0,
    ];
    final rebounds = [
      for (final row in logs.reversed)
        _rawNumber(row, const ['trb', 'reb', 'rebounds']) ?? 0,
    ];
    final assists = [
      for (final row in logs.reversed)
        _rawNumber(row, const ['ast', 'assists']) ?? 0,
    ];
    return _ToolSurface(
      title: 'Last X Games',
      subtitle:
          'Recent-form analysis uses only available game-log rows; missing games remain missing.',
      controls: [
        _PlayerDrop(rows: rows, value: playerId, onChanged: onPlayer),
        _SmallDrop<int>(
          value: lastGames,
      ...
      child: logs.isEmpty
          ? const _EmptyTool(
              'No player-game rows are available for this player in the current asset release.',
            )
          : Column(
              children: [
                _MetricCards(
                  items: [
                    _MetricCard(
                      'PTS AVG',
                      _average(points).toStringAsFixed(1),
                      null,
                    ),
                    _MetricCard(
                      'REB AVG',
                      _average(rebounds).toStringAsFixed(1),
                      null,
                    ),
                    _MetricCard(
                      'AST AVG',
                      _average(assists).toStringAsFixed(1),
                      null,
                    ),
                ...
                SizedBox(
                  ...
                  child: _SparklinePanel(
                    series: {'PTS': points, 'REB': rebounds, 'AST': assists},
                  ),
                ),
                const SizedBox(height: 10),
                _GameLogTable(logs: logs),
  ...
  Widget build(BuildContext context) {
    ...
    final segments = [
      _ProfileSegment(
        'Two-point volume',
        twoA / total,
        player?.value('two_pct'),
      ),
      _ProfileSegment(
        'Three-point volume',
        threeA / total,
        player?.value('three_pct'),
      ),
      _ProfileSegment(
        'Free-throw pressure',
        .44 * fta / total,
    ...
    return _ToolSurface(
      title: 'Shot Profile',
      subtitle:
          'A source-backed shot-mix view from two-point, three-point and free-throw box-score data.',
          ...
          : Column(
              children: [
                for (final segment in segments)
                  _ShotProfileBar(segment: segment),
                const SizedBox(height: 10),
                const _SourceNotice(
                  'Rim, midrange, openness and shot-quality components require location/tracking data and are not inferred from aggregate box scores.',
                ),
              ],
            ),
    );
  ...
  Widget build(BuildContext context) {
    final roster =
        rows
            ...
            .toList()
          ..sort(
            (a, b) => (b.value('min') ?? 0).compareTo(a.value('min') ?? 0),
          );
    final selected = roster
        .where((row) => selectedIds.contains(row.playerId))
        .toList();
    return _ToolSurface(
      title: 'Lineup Builder',
      subtitle:
          'Select up to five players. Aggregates are honest roster-production sums, not fabricated on/off or RAPM estimates.',
      controls: [
        _SmallDrop<String>(
          value: team,
          values: teams,
        ...
        children: [
            ...
            children: [
              for (final row in roster)
                FilterChip(
                  ...
                  label: Text(
                    row.player,
                    style: const TextStyle(color: _suiteText, fontSize: 10),
                  ),
                ),
            ],
          ),
          ...
          _MetricCards(
            items: [
              ...
              _MetricCard(
                'PTS SUM',
                _sum(selected, 'pts').toStringAsFixed(1),
                null,
              ),
              _MetricCard(
                'AST SUM',
                _sum(selected, 'ast').toStringAsFixed(1),
                null,
              ),
              _MetricCard(
                'PROD SUM',
                _sum(selected, 'game_score_proxy').toStringAsFixed(1),
                null,
              ),
            ],
          ),
          const SizedBox(height: 10),
          const _SourceNotice(
            'True together/apart performance, possessions and lineup net rating will activate only when normalized lineup stints are present.',
          ),
        ],
      ),
    );
...
class _TierListTool extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final tiers = <String, List<NbaStatsRow>>{
      'S · 95th percentile+': ranked
          .where((row) => (row.percentiles[metricKey] ?? 0) >= 95)
          .toList(),
      'A · 85–94': ranked
          .where(
            (row) =>
                (row.percentiles[metricKey] ?? 0) >= 85 &&
                (row.percentiles[metricKey] ?? 0) < 95,
          )
          .toList(),
      'B · 70–84': ranked
          .where(
            (row) =>
                (row.percentiles[metricKey] ?? 0) >= 70 &&
                (row.percentiles[metricKey] ?? 0) < 85,
          )
          .toList(),
      'C · 50–69': ranked
          .where(
            (row) =>
                (row.percentiles[metricKey] ?? 0) >= 50 &&
                (row.percentiles[metricKey] ?? 0) < 70,
          )
          .toList(),
      'D · below 50': ranked
          .where((row) => (row.percentiles[metricKey] ?? 0) < 50)
          .toList(),
    };
    return _ToolSurface(
      title: 'Tier List',
      subtitle:
          'Automatic tiers from transparent percentile rules; no hidden editorial score.',
      ...
      child: Column(
        children: [
          for (final entry in tiers.entries)
            _TierBand(
              title: entry.key,
              players: entry.value.take(25).toList(),
              metricKey: metricKey,
              engine: engine,
            ),
        ],
...
class _OffensiveRatingSandbox extends StatefulWidget {
  ...
  @override
  State<_OffensiveRatingSandbox> createState() =>
      _OffensiveRatingSandboxState();
}
...
class _OffensiveRatingSandboxState extends State<_OffensiveRatingSandbox> {
  ...
  double get rating {
    ...
    final secondChanceBonus =
        fieldGoalAttempts *
        (1 - (twoPct * (1 - threeRate) + threePct * threeRate)) *
  ...
  Widget build(BuildContext context) => _ToolSurface(
    ...
    subtitle:
        'Move transparent possession levers and see the modeled points per 100 possessions.',
    controls: const [],
    child: Column(
      children: [
        _MetricCards(
          items: [
            ...
            _MetricCard(
              'TOV%',
              '${(turnoverRate * 100).toStringAsFixed(1)}%',
              null,
            ),
          ],
        ),
        ...
        _Slider(
          ...
          .10,
          .70,
          (value) => setState(() => threeRate = value),
        ),
        _Slider(
          ...
          .05,
          .60,
          (value) => setState(() => freeThrowRate = value),
        ),
        _Slider(
          ...
          .50,
          .95,
          (value) => setState(() => freeThrowPct = value),
        ),
        _Slider(
          'Turnover rate',
          turnoverRate,
          .05,
          .25,
          (value) => setState(() => turnoverRate = value),
          lowerBetter: true,
        ),
        _Slider(
          ...
          .10,
          .45,
          (value) => setState(() => offensiveReboundRate = value),
        ),
        const _SourceNotice(
          'This sandbox is a transparent possession model, not an official NBA offensive-rating formula or a prediction of a specific team.',
        ),
      ],
    ),
  );
  ...
  Widget build(BuildContext context) {
    final checks = <_CoverageItem>[
      _CoverageItem(
        ...
        data.players.length,
        true,
        'Sourced player identity rows',
      ),
      _CoverageItem(
        'Player season summaries',
        data.playerSeasonTotals.length,
        true,
        'Core Stats and rankings input',
      ),
      _CoverageItem(
        'Games',
        data.games.length,
        true,
        'Schedule and results layer',
      ),
      _CoverageItem(
        'Player game logs',
        data.playerGameLogsTop.length,
        data.playerGameLogsTop.isNotEmpty,
        'Recent-form and trend layer',
      ),
      _CoverageItem(
        'Team records',
        data.teamRecords.length,
        true,
        'Team comparison context',
      ),
      _CoverageItem(
        'Standings',
        data.standings.length,
        data.standings.isNotEmpty,
        'Conference and playoff context',
      ),
      _CoverageItem(
        'Normalized play-by-play',
        data.playByPlayEvents,
        data.playByPlayEvents > 0,
        'Required for possession research',
      ),
    ];
    return _ToolSurface(
      title: 'Data Coverage',
      subtitle:
          'Inspect the actual release before interpreting a missing tool as a zero or negative result.',
      controls: const [],
      child: Column(
        children: [
          ...
          const SizedBox(height: 10),
          _SourceNotice(
            'Dataset status: ${data.datasetStatus} · Validation: ${data.validationStatus} · Assets: ${data.assetPath}',
          ),
        ],
      ),
    );
...
class _MethodologyGate extends StatelessWidget {
  ...
  Widget build(BuildContext context) => _ToolSurface(
    title: tool,
    subtitle:
        'The interface contract is reserved, but Sports Terminal will not publish synthetic tracking or RAPM output.',
    controls: const [],
    child: const Column(
      children: [
        ...
        Text(
          'SOURCE-BOUND MODULE',
          style: TextStyle(color: _suiteText, fontWeight: FontWeight.w900),
        ),
        SizedBox(height: 8),
        Text(
          'This module activates when its normalized source tables, methodology version, validation report and provenance record are present. Until then, the product explains the requirement instead of filling the screen with unsupported estimates.',
          textAlign: TextAlign.center,
          style: TextStyle(color: _suiteMuted, height: 1.5),
        ),
      ],
    ),
  );
...
class _SuiteSurface extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    decoration: BoxDecoration(
      ...
      border: Border.all(color: _suiteLine),
    ),
    child: child,
  );
}
...
InputDecoration _suiteInput(String label) => InputDecoration(
  ...
  border: OutlineInputBorder(
    ...
    borderSide: const BorderSide(color: _suiteLine),
  ),
        ...
        Padding(
          ...
          child: Row(
            children: [
              ...
              Expanded(
                child: _PercentileTrack(value: row.percentiles[key] ?? 0),
              ),
              const SizedBox(width: 8),
              SizedBox(
                width: 62,
...
class _ComparisonBar extends StatelessWidget {
  const _ComparisonBar({
    ...
    required this.percentileA,
    required this.percentileB,
    required this.nameA,
    required this.nameB,
  });
  final String label;
          ...
          child: Column(
            ...
            children: [
              ...
              Text(
                ...
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
                style: const TextStyle(color: _suiteMuted, fontSize: 8),
              ),
            ],
          ),
        ),
        Expanded(
          child: Column(
            children: [
              ...
              Row(
                children: [
                  Expanded(child: _PercentileTrack(value: percentileA)),
                  const SizedBox(width: 5),
                  Text(
                    '${percentileA.round()}',
                    style: const TextStyle(color: _suiteYellow, fontSize: 8),
                  ),
                  const SizedBox(width: 10),
                  Text(
                    '${percentileB.round()}',
                    style: const TextStyle(color: _suiteCyan, fontSize: 8),
                  ),
                  const SizedBox(width: 5),
                  Expanded(child: _PercentileTrack(value: percentileB)),
                ],
              ),
            ],
          ),
        ...
        for (final row in logs)
          _GameRow(
            values: [
              ...
              _displayNumber(
                _rawNumber(row, const ['plus_minus', 'plusMinus']),
                signed: true,
              ),
            ],
          ),
...
class _GameRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    decoration: const BoxDecoration(
      border: Border(bottom: BorderSide(color: _suiteLine, width: .5)),
    ),
        ...
        for (final value in values)
          Expanded(
            child: Text(
              ...
              textAlign: TextAlign.center,
              style: TextStyle(
                color: header ? _suiteMuted : _suiteText,
      ...
      children: [
        SizedBox(
          ...
          child: Text(
            '#$index',
            style: TextStyle(
              color: index <= 3 ? _suiteYellow : _suiteMuted,
              fontWeight: FontWeight.w900,
            ),
          ),
        ),
        ...
        SizedBox(
          ...
          child: Text(
            ...
            style: const TextStyle(color: _suiteMuted, fontSize: 9),
          ),
        ),
        SizedBox(
          width: 80,
...
class _TeamComparisonRow extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final aWins = metric.lowerBetter
        ? metric.a < metric.b
        : metric.a > metric.b;
    final bWins = metric.lowerBetter
        ? metric.b < metric.a
        : metric.b > metric.a;
    return Container(
      ...
      child: Row(
        children: [
          SizedBox(
            width: 100,
            child: Text(
              '$teamA  ${metric.a.toStringAsFixed(1)}',
              style: TextStyle(
                color: aWins ? _suiteYellow : _suiteText,
                fontWeight: FontWeight.w900,
              ),
            ),
          ),
          ...
          SizedBox(
            width: 100,
            child: Text(
              '${metric.b.toStringAsFixed(1)}  $teamB',
              textAlign: TextAlign.right,
              style: TextStyle(
                color: bWins ? _suiteYellow : _suiteText,
                fontWeight: FontWeight.w900,
              ),
            ),
          ),
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                Text(
                  '${(row.value('pts') ?? 0).toStringAsFixed(1)} PTS',
                  style: const TextStyle(color: _suiteMuted, fontSize: 9),
                ),
              ],
            ),
      ...
      children: [
        Row(
          children: [
            ...
            Text(
              '${(segment.share * 100).toStringAsFixed(1)}% volume',
              style: const TextStyle(color: _suiteYellow),
            ),
            const SizedBox(width: 12),
            Text(
              segment.efficiency == null
                  ? '—'
                  : '${(segment.efficiency! * 100).toStringAsFixed(1)}% accuracy',
              style: const TextStyle(color: _suiteCyan),
            ),
          ],
        ),
        const SizedBox(height: 8),
        Stack(
          children: [
            ...
            FractionallySizedBox(
              widthFactor: segment.share.clamp(0, 1),
              child: Container(
                height: 12,
                decoration: BoxDecoration(
                  color: _suiteYellow,
        ...
        Wrap(
          ...
          children: [
            for (final row in players)
              Chip(
                backgroundColor: _suiteBg,
                side: const BorderSide(color: _suiteLine),
                label: Text(
                  '${row.player} · ${engine.formatValue(metricKey, row.value(metricKey))}',
                  style: const TextStyle(color: _suiteText, fontSize: 9),
                ),
              ),
          ],
        ),
  ...
  Widget build(BuildContext context) => Row(
    children: [
      SizedBox(
        ...
        child: Text(
          label,
          style: const TextStyle(color: _suiteMuted, fontSize: 10),
        ),
      ),
      Expanded(
        child: Slider(
          ...
          min: min,
          max: max,
          activeColor: lowerBetter ? _suiteCyan : _suiteYellow,
          onChanged: onChanged,
        ),
      ),
      SizedBox(
...
class _SourceNotice extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    padding: const EdgeInsets.all(10),
    decoration: BoxDecoration(
      color: const Color(0xFF263447),
      borderRadius: BorderRadius.circular(8),
      border: Border.all(color: _suiteCyan.withValues(alpha: .35)),
    ),
    child: Text(
      text,
      style: const TextStyle(color: _suiteCyan, fontSize: 9, height: 1.4),
    ),
  );
}
...
class _SparklinePanel extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: CustomPaint(
      painter: _MultiSparkPainter(series),
...
class _MultiSparkPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final values in series.values) {
      ...
      for (var i = 0; i < values.length; i++) {
        final x =
            25 +
            (values.length == 1 ? .5 : i / (values.length - 1)) *
                (size.width - 35);
        final y =
            size.height -
            20 -
            (values[i] - min) / math.max(.0001, max - min) * (size.height - 35);
        if (i == 0) {
          path.moveTo(x, y);
        } else {
          path.lineTo(x, y);
  ...
  @override
  bool shouldRepaint(covariant _MultiSparkPainter oldDelegate) =>
...
class _CoverageRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Row(
      children: [
        Icon(
          item.available ? Icons.check_circle : Icons.warning_amber_rounded,
          color: item.available ? _suiteGreen : _suiteYellow,
          size: 18,
        ),
  ...
  Widget build(BuildContext context) => Padding(
    ...
    child: Center(
      child: Text(
        message,
        textAlign: TextAlign.center,
        style: const TextStyle(color: _suiteMuted),
      ),
    ),
  );
}
...
Map<String, dynamic> _teamRecord(
...
double _recordValue(Map<String, dynamic> row, List<String> keys) =>
    _rawNumber(row, keys) ?? 0;

double _sum(List<NbaStatsRow> rows, String key) =>
    rows.fold(0, (sum, row) => sum + (row.value(key) ?? 0));

double _topSum(List<NbaStatsRow> rows, String key, int count) {
  final copy = [...rows]
    ..sort((a, b) => (b.value(key) ?? 0).compareTo(a.value(key) ?? 0));
