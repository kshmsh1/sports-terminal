...
class _ProductNbaStatsWorkstationScreenState
  ...
  Future<void> _copyRows(List<NbaStatsRow> rows) async {
    ...
    final header = [
      ...
      'Team',
      'Pos',
      for (final key in keys) _metric(key).shortLabel,
    ];
  ...
  Future<void> _openCompare(List<NbaStatsRow> allRows) async {
    final selected =
        allRows.where((row) => _compareIds.contains(row.playerId)).toList();
          ...
          Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                _StatusTag(
                  text: seed.usedFallback
                      ? 'FALLBACK SEED'
                      : seed.datasetStatus.toUpperCase(),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          _StatusTag(
            text: '$available/${visibleKeys.length} COLUMNS POPULATED',
            tone: available == visibleKeys.length ? _green : _amber,
          ),
...
class _StatsTable extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    const playerWidth = 226.0;
    const teamWidth = 70.0;
    const posWidth = 50.0;
    const metricWidth = 86.0;
    final width =
        playerWidth + teamWidth + posWidth + metricWidth * metricKeys.length;
    return Container(
      decoration: BoxDecoration(
        color: _surface,
        border: Border.all(color: _stroke),
      ...
      child: rows.isEmpty
          ? const SizedBox(
              ...
              child: Center(
                child: Text(
                  'No players match the current segment and filters.',
...
class _TableHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(
          children: [
            const _StaticHeader(width: 226, label: 'PLAYER'),
            const _StaticHeader(width: 70, label: 'TEAM'),
            const _StaticHeader(width: 50, label: 'POS'),
            for (final key in metricKeys)
              _MetricHeader(
                ...
                onSort: () => onSort(key),
                onExpand:
                    _metric(key).children.isEmpty ? null : () => onExpand(key),
              ),
          ],
        ),
      );
    ...
    return Container(
      ...
      child: Row(
        children: [
                  ...
                  InkWell(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color: comparing ? _blue : _faint,
                        ),
                      ),
                      child: comparing
                          ? const Icon(Icons.check, color: _blue, size: 11)
                          : null,
                    ),
                  ),
          ...
          _TextCell(width: 70, text: row.team),
          _TextCell(width: 50, text: row.position),
          for (final key in metricKeys)
            _MetricCell(
...
class _MetricHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        width: 86,
        decoration: const BoxDecoration(
          border: Border(left: BorderSide(color: _strokeSoft)),
        ),
        child: InkWell(
          onTap: onSort,
          child: Padding(
              ...
              child: Row(
                children: [
                  Container(
                    width: 2,
                    height: 14,
                    color: entry.key == activeFamily.label ? _blue : _faint,
                  ),
                  const SizedBox(width: 7),
                  Text(
              ...
              builder: (context, constraints) {
                final columns = constraints.maxWidth >= 1200
                    ...
                    : constraints.maxWidth >= 720
                        ...
                        : 1;
                final width =
                    (constraints.maxWidth - 28 - (columns - 1) * 8) / columns;
                return Padding(
                    ...
                    children: [
                      TableRow(
                        children: [
                          ...
                          for (final row in rows)
                            _CompareCell(text: row.player, header: true),
                        ],
                      ),
                      for (final key in metricKeys)
                        TableRow(
                          children: [
                            ...
                            for (final row in rows)
                              _CompareCell(text: resolver.format(row, key)),
                          ],
                        ),
                    ],
                  ),
