...
class GamesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _AssetWorkspaceScreen<GameRecord>(
      title: 'Games',
      subtitle: 'Game command center for schedules, results, box-score hooks, game logs, playoff games, matchup context, and future chart-ready game-level data.',
      ...
      emptyBody: 'The game records asset is connected and empty. Once schedule, result, and box-score source paths are approved, this page can become the bridge between team pages, player game logs, season pages, playoff series, and report generation.',
      ...
      factBuilder: (game) => [
        _RecordFact('Game ID', game.id),
        _RecordFact('Season', game.seasonId),
        _RecordFact('Season Type', game.seasonType ?? '—'),
        _RecordFact('Date', game.gameDate ?? '—'),
        _RecordFact('Away Team', game.awayTeamId ?? '—'),
        _RecordFact('Home Team', game.homeTeamId ?? '—'),
        _RecordFact('Score', _score(game)),
        _RecordFact('Arena / City', '${game.arena ?? '—'} / ${game.city ?? '—'}'),
        _RecordFact('Source', game.sourceId ?? 'Source pending'),
        _RecordFact('asOf', game.asOf ?? '—'),
      ],
      activationSteps: const [
        _ActivationStep('Schedule backbone', 'Add game date, season, home team, away team, arena, city, and season type.'),
        _ActivationStep('Result layer', 'Add home score, away score, final status, overtime flags, and source metadata.'),
        _ActivationStep('Box-score layer', 'Attach player game stats and team game stats after season stats are stable.'),
        _ActivationStep('Playoff linkage', 'Connect playoff series, game number, series score, round, seeds, and bracket context.'),
        _ActivationStep('Chart layer', 'Use game rows to power within-season metric trend charts.'),
      ],
      leadTitle: 'Game Center Principle',
      leadBody: 'Games should become the bridge between season summaries and granular player/team performance. The MVP can start with schedules and results, then grow into player game logs, team game logs, matchup pages, playoff series detail, and finance-style trend charts.',
    );
  }
...
class RostersScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _AssetWorkspaceScreen<RosterEntry>(
      title: 'Rosters',
      subtitle: 'Roster command center for team-season rosters, player-team windows, two-way status, active/inactive context, assignments, recalls, lineup context, and development pathways.',
      ...
      emptyBody: 'The roster entries asset is connected and empty. Once roster snapshots or transaction-derived roster data are approved, this page will connect players, teams, seasons, games, transactions, awards, and G League development.',
      ...
      factBuilder: (entry) => [
        _RecordFact('Player ID', entry.playerId),
        _RecordFact('Team ID', entry.teamId),
        _RecordFact('Season', entry.seasonId),
        _RecordFact('Jersey', entry.jerseyNumber ?? '—'),
        _RecordFact('Position', entry.position ?? '—'),
        _RecordFact('Roster Status', entry.rosterStatus ?? '—'),
        _RecordFact('Contract Type', entry.contractType ?? '—'),
        _RecordFact('Active Window', '${entry.startDate ?? '—'} to ${entry.endDate ?? '—'}'),
        _RecordFact('Source', entry.sourceId ?? 'Source pending'),
        _RecordFact('asOf', entry.asOf ?? '—'),
      ],
      activationSteps: const [
        _ActivationStep('Team-season snapshot', 'Start with playerId, teamId, seasonId, position, jersey number, and source metadata.'),
        _ActivationStep('Roster windows', 'Add start dates, end dates, active status, inactive status, and two-way status.'),
        _ActivationStep('Transaction linkage', 'Use transaction rows to explain roster changes instead of only showing static snapshots.'),
        _ActivationStep('Game eligibility', 'Later connect roster windows to game logs, box scores, injuries, and availability context.'),
        _ActivationStep('G League pathway', 'Track assignments, recalls, two-way movement, and player development arcs.'),
      ],
      leadTitle: 'Roster Graph Principle',
      leadBody: 'Rosters are the connective tissue between players, teams, seasons, transactions, games, contracts, and G League development paths. This module should eventually answer who was on a team, when they were available, how they joined, and how their role changed.',
    );
  }
}
...
class AwardsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _AssetWorkspaceScreen<AwardRecord>(
      ...
      emptyBody: 'The awards asset is connected and empty. The target is not just winners. The data model already supports rank, first-place votes, points, and share, so awards can become full award-race boards once historical voting rows are sourced.',
      ...
      factBuilder: (award) => [
        _RecordFact('Award', award.awardName),
        _RecordFact('Season', award.seasonId),
        _RecordFact('Player ID', award.playerId ?? '—'),
        _RecordFact('Team ID', award.teamId ?? '—'),
        _RecordFact('Race Rank', award.rank?.toString() ?? 'Winner/finalist rank pending'),
        _RecordFact('First-Place Votes', award.votesFirstPlace?.toString() ?? '—'),
        _RecordFact('Voting Points', award.points?.toStringAsFixed(1) ?? '—'),
        _RecordFact('Vote Share', award.share?.toStringAsFixed(3) ?? '—'),
        _RecordFact('Source', award.sourceId ?? 'Source pending'),
        _RecordFact('asOf', award.asOf ?? '—'),
      ],
      activationSteps: const [
        _ActivationStep('Winner records', 'Load major award winners by season, player, team, and source.'),
        _ActivationStep('Race records', 'Load runners-up, finalists, rank, vote points, first-place votes, and vote share.'),
        _ActivationStep('Season boards', 'Show each season’s MVP, DPOY, ROY, MIP, Sixth Man, All-NBA, All-Defense, and All-Rookie races.'),
        _ActivationStep('Player award profile', 'Attach wins, finalist finishes, vote shares, and award history to player pages.'),
      ...
      leadBody: 'Awards should preserve the full race, not only the winner. The end platform should show who won, who finished behind, how votes broke down, what team context mattered, and how each race links back to player, team, season, compare, and report pages.',
    );
  }
}\n
class DraftScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _AssetWorkspaceScreen<DraftPick>(
      ...
      factBuilder: (pick) => [
        _RecordFact('Draft Year', pick.draftYear.toString()),
        _RecordFact('Round', pick.round?.toString() ?? '—'),
        _RecordFact('Pick Number', pick.pickNumber?.toString() ?? '—'),
        _RecordFact('Team ID', pick.teamId ?? '—'),
        _RecordFact('Player', pick.playerName ?? pick.playerId ?? '—'),
        _RecordFact('Player ID', pick.playerId ?? 'Identity link pending'),
        _RecordFact('School / Club', pick.schoolOrClub ?? '—'),
        _RecordFact('Country', pick.country ?? '—'),
        _RecordFact('Source', pick.sourceId ?? 'Source pending'),
        _RecordFact('asOf', pick.asOf ?? '—'),
      ],
      activationSteps: const [
        _ActivationStep('Draft board', 'Load year, round, pick, team, player, school/club, country, and source metadata.'),
        _ActivationStep('Player identity join', 'Attach drafted players to player profiles once player identity is populated.'),
        _ActivationStep('Outcome layer', 'Connect picks to rookie season, career stats, awards, transactions, and team tenure.'),
        _ActivationStep('Team strategy', 'Summarize team draft history, positional focus, hit rate, and retained value.'),
        _ActivationStep('Development path', 'Connect G League assignments, roster windows, and player progression.'),
      ],
...
class TransactionsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return _AssetWorkspaceScreen<TransactionRecord>(
      ...
      recordKey: (tx) => tx.id,
      factBuilder: (tx) => [
        _RecordFact('Transaction ID', tx.id),
        _RecordFact('Date', tx.date ?? '—'),
        _RecordFact('Type', tx.transactionType ?? '—'),
        _RecordFact('Player', tx.playerName ?? tx.playerId ?? '—'),
        _RecordFact('Player ID', tx.playerId ?? 'Identity link pending'),
        _RecordFact('From Team', tx.fromTeamId ?? '—'),
        _RecordFact('To Team', tx.toTeamId ?? '—'),
        _RecordFact('Description', tx.description ?? '—'),
        _RecordFact('Source', tx.sourceId ?? 'Source pending'),
        _RecordFact('asOf', tx.asOf ?? '—'),
      ],
      activationSteps: const [
        _ActivationStep('Movement event', 'Load date, type, player, from team, to team, description, and source metadata.'),
        _ActivationStep('Roster effect', 'Connect each movement event to roster windows and team-season pages.'),
        _ActivationStep('Trade tree', 'Later connect multi-player trades, picks, protections, cash, exceptions, and downstream outcomes.'),
        _ActivationStep('Contract context', 'Connect signings, extensions, waivers, and option decisions to contract pages.'),
        _ActivationStep('Timeline reports', 'Generate player movement timelines, team transaction histories, and franchise-building reports.'),
      ],
    ...
    return FutureBuilder<List<T>>(
      ...
      builder: (context, snapshot) {
        ...
        final filtered = records.where((record) {
          ...
          final matchesQuery = normalized.isEmpty || values.join(' ').toLowerCase().contains(normalized);
          final matchesStatus = statusFilter == 'All' || (statusFilter == 'Sourced' && sourceValue != '—') || (statusFilter == 'Source pending' && sourceValue == '—');
        ...
        return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          LayoutBuilder(builder: (context, constraints) {
            ...
            return GridView.count(crossAxisCount: isWide ? 4 : 2, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), crossAxisSpacing: 14, mainAxisSpacing: 14, childAspectRatio: isWide ? 2.0 : 1.5, children: [
              _WorkspaceMetric(label: widget.recordLabel, value: '${records.length}', detail: records.isEmpty ? 'Source pending' : 'Loaded from asset'),
              _WorkspaceMetric(label: 'Visible', value: '${filtered.length}', detail: 'After filters'),
              _WorkspaceMetric(label: 'Sourced', value: '$sourced', detail: 'Rows with source metadata'),
              _WorkspaceMetric(label: 'Schema / Planned / Future', value: '$schemaReady / $planned / $future', detail: 'Build map'),
            ]);
          }),
          const SizedBox(height: 22),
          ...
          TerminalCard(child: Wrap(spacing: 12, runSpacing: 12, children: [
            SizedBox(width: 360, child: TextField(onChanged: (value) => setState(() => query = value), style: const TextStyle(color: Colors.white), cursorColor: terminalAccent, decoration: _inputDecoration('Search ${widget.title.toLowerCase()} records...'))),
            _FilterDropdown(label: 'Source Status', value: statusFilter, values: const ['All', 'Sourced', 'Source pending'], onChanged: (value) => setState(() => statusFilter = value)),
          ])),
          const SizedBox(height: 22),
          LayoutBuilder(builder: (context, constraints) {
...
class _ActivationStep {
  const _ActivationStep(this.title, this.body);
  final String title;
  final String body;
}
...
class _SelectedRecordPanel<T> extends StatelessWidget {
  const _SelectedRecordPanel({required this.title, required this.record, required this.factBuilder});
  final String title;
  final T? record;
  final List<_RecordFact> Function(T record) factBuilder;
  @override
  Widget build(BuildContext context) {
    if (record == null) return TerminalCard(child: Text('Select a $title record to inspect details, source status, and downstream links. This asset is currently source-pending.', style: const TextStyle(color: terminalTextSoft, height: 1.45)));
    final facts = factBuilder(record as T);
    return TerminalCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('Selected $title Record', style: const TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800)),
      const SizedBox(height: 12),
      for (final fact in facts) _FactLine(label: fact.label, value: fact.value),
    ]));
  }
}
...
class _ActivationPanel extends StatelessWidget {
  const _ActivationPanel({required this.title, required this.steps});
  final String title;
...
class _RecordTable<T> extends StatelessWidget {
  const _RecordTable({required this.title, required this.records, required this.columns, required this.rowBuilder, required this.recordKey, required this.selectedKey, required this.onSelected});
  final String title;
