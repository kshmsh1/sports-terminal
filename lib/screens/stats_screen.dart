...
import '../data/stats_command_stage_items.dart';
...
class _StatsScreenState extends State<StatsScreen> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<_StatsPayload>(
      ...
      builder: (context, snapshot) {
        ...
        final stageCategories = ['All', ...statsCommandStageItems.map((item) => item.category).toSet().toList()..sort()];
        final filteredStages = statsCommandStageItems.where((item) => stageCategory == 'All' || item.category == stageCategory).toList();
        final p0Stages = statsCommandStageItems.where((item) => item.priority == 'P0').length;

        return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const SectionHeader(title: 'Stats', subtitle: 'Statistics command workspace for player and team season tables, metric families, rate view modes, regular season/playoff splits, advanced efficiency fields, leaderboards, chart planning, route actions, and source-aware empty states.'),
          const SizedBox(height: 22),
          _MetricGrid(metrics: [_MetricSpec('Player Stat Rows', '${payload.playerStats.length}', payload.playerStats.isEmpty ? 'Source pending' : 'Loaded'), _MetricSpec('Team Stat Rows', '${payload.teamStats.length}', payload.teamStats.isEmpty ? 'Source pending' : 'Loaded'), _MetricSpec('Metric Families', '${statsMetricFamilyItems.length}', '$p0Stages P0 stages'), _MetricSpec('Visible Rows', '${visiblePlayerRows.length + visibleTeamRows.length}', '$viewMode / $route')]),
          const SizedBox(height: 22),
          TerminalCard(child: Wrap(spacing: 12, runSpacing: 12, children: [
            ...
            _FilterDropdown(label: 'Season Type', value: seasonType, values: const ['All', 'Regular Season', 'Playoffs', 'Play-In'], onChanged: (value) => setState(() => seasonType = value)),
            _FilterDropdown(label: 'Min GP', value: minGames, values: const ['All', '1+', '20+', '41+', '65+'], onChanged: (value) => setState(() => minGames = value)),
            _FilterDropdown(label: 'Family', value: statFamily, values: ['All', ...statsMetricFamilyItems.map((item) => item.family)], onChanged: (value) => setState(() => statFamily = value)),
            ...
            _FilterDropdown(label: 'Route', value: route, values: _statRoutes.map((item) => item.name).toList(), onChanged: (value) => setState(() => route = value)),
            _FilterDropdown(label: 'Stage', value: stageCategory, values: stageCategories, onChanged: (value) => setState(() => stageCategory = value)),
          ])),
          const SizedBox(height: 22),
          _StatsCommandTicket(family: statFamily, viewMode: viewMode, seasonType: seasonType, route: _statRoutes.firstWhere((item) => item.name == route), coverage: coverage, playerRows: visiblePlayerRows.length, teamRows: visibleTeamRows.length),
          const SizedBox(height: 22),
          ...
          const SizedBox(height: 22),
          const _StatsRouteMatrix(),
          const SizedBox(height: 22),
          const _MetricFamilyRegistryTable(),
          const SizedBox(height: 22),
          const _ChartPlanningPanel(),
          const SizedBox(height: 22),
          _StatsCommandStageTable(items: filteredStages),
          const SizedBox(height: 22),
  ...
  bool _matchesPlayer(PlayerSeasonStat row, _StatsPayload payload, String normalized) {
    ...
    if (seasonType != 'All' && row.seasonType != seasonType) return false;
  ...
  int? _minimumGames(String value) { if (value == '1+') return 1; if (value == '20+') return 20; if (value == '41+') return 41; if (value == '65+') return 65; return null; }
}
...
class _StatRoute { const _StatRoute(this.name, this.status, this.target, this.use); final String name; final String status; final String target; final String use; }
const _statRoutes = <_StatRoute>[
  _StatRoute('Workspace', 'Planned', 'Workspace Studio', 'Send selected stat rows, filters, metric family, and source snapshot into custom tables.'),
  _StatRoute('Compare', 'Planned', 'Compare', 'Compare selected players, teams, seasons, metric packages, and saved views.'),
  _StatRoute('Report', 'Planned', 'Reports', 'Generate player, team, season, leaderboard, or award-case report sections.'),
  _StatRoute('Export', 'Planned', 'Export Center', 'Export filtered stat tables with source notes and missing-data flags.'),
  _StatRoute('Save View', 'Planned', 'Saved Views', 'Persist filters, visible columns, selected family, route intent, and source snapshot.'),
  _StatRoute('Alert', 'Future', 'Alerts', 'Monitor leaderboard movement, threshold changes, rank changes, and source updates.'),
  _StatRoute('Fantasy', 'Future', 'Fantasy Terminal', 'Route player stats into roster, waiver, trade, projection, and matchup workflows.'),
  _StatRoute('Source Audit', 'Planned', 'Data Ops', 'Inspect provenance, missing fields, validation status, and rights posture.'),
];
...
class _StatsCommandTicket extends StatelessWidget { const _StatsCommandTicket({required this.family, required this.viewMode, required this.seasonType, required this.route, required this.coverage, required this.playerRows, required this.teamRows}); final String family; final String viewMode; final String seasonType; final _StatRoute route; final _StatsCoverage coverage; final int playerRows; final int teamRows; @override Widget build(BuildContext context) => TerminalCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Stats Command Ticket', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800)), const SizedBox(height: 10), const Text('Stats is the quantitative backbone of the terminal. This command layer keeps filters, metric families, rate modes, selected rows, and route intent attached to every analysis object.', style: TextStyle(color: terminalTextSoft, height: 1.45)), const SizedBox(height: 14), Wrap(spacing: 10, runSpacing: 10, children: [InfoPill(label: family), InfoPill(label: viewMode), InfoPill(label: seasonType), InfoPill(label: route.status), InfoPill(label: '$playerRows player rows'), InfoPill(label: '$teamRows team rows')]), const SizedBox(height: 12), _DetailLine(label: 'Route', value: '${route.name} → ${route.target}. ${route.use}'), _DetailLine(label: 'Join Coverage', value: '${coverage.joinedRows} joined row checks across player/team/season IDs.'), _DetailLine(label: 'Source Coverage', value: '${coverage.rowsWithSource} rows with source IDs.') ])); }

class _StatsReadinessPanel extends StatelessWidget { const _StatsReadinessPanel({required this.payload, required this.coverage}); final _StatsPayload payload; final _StatsCoverage coverage; @override Widget build(BuildContext context) => TerminalCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Stats MVP Readiness', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800)), const SizedBox(height: 10), const Text('Stats is being shaped around source-backed tables, stat families, regular season/playoff splits, rate view modes, chart readiness, and clean joins. Advanced and tracking metrics stay source-pending until the right data path exists.', style: TextStyle(color: terminalTextSoft, height: 1.45)), const SizedBox(height: 18), Wrap(spacing: 10, runSpacing: 10, children: [InfoPill(label: '${payload.players.length} players'), InfoPill(label: '${payload.teams.length} teams'), InfoPill(label: '${payload.seasons.length} seasons'), InfoPill(label: '${payload.playerStats.length} player stat rows'), InfoPill(label: '${payload.teamStats.length} team stat rows'), InfoPill(label: '${coverage.rowsWithSource} sourced rows')])])); }
class _StatFamilyPanel extends StatelessWidget { const _StatFamilyPanel({required this.statFamily, required this.viewMode}); final String statFamily; final String viewMode; @override Widget build(BuildContext context) { final selected = statsMetricFamilyItems.where((item) => item.family == statFamily).firstOrNull; return TerminalCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Text('Stat Family + View Mode Layer', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800)), const SizedBox(height: 10), Text(selected == null ? 'Current family: $statFamily. Current view: $viewMode. The table can be organized around metric families rather than one giant metric dump.' : '${selected.family}: ${selected.firstUse}', style: const TextStyle(color: terminalTextSoft, height: 1.45)), const SizedBox(height: 16), const Wrap(spacing: 10, runSpacing: 10, children: [InfoPill(label: 'Regular season'), InfoPill(label: 'Playoffs'), InfoPill(label: 'Per game'), InfoPill(label: 'Per 36'), InfoPill(label: 'Per 48'), InfoPill(label: 'Per 100'), InfoPill(label: 'Totals')])])); } }

class _SelectedStatPanel extends StatelessWidget { const _SelectedStatPanel({required this.detail}); final _SelectedStatDetail? detail; @override Widget build(BuildContext context) { if (detail == null) return const TerminalCard(child: Text('Select a stat row to inspect joined identity, season, source, and metric detail. Stat assets are currently source-pending.', style: TextStyle(color: terminalTextSoft, height: 1.45))); final playerRow = detail!.playerRow; final teamRow = detail!.teamRow; final payload = detail!.payload; final title = playerRow != null ? payload.playersById[playerRow.playerId]?.displayName ?? playerRow.playerId : payload.teamsById[teamRow!.teamId]?.name ?? teamRow.teamId; final subtitle = playerRow != null ? '${payload.seasonsById[playerRow.seasonId]?.label ?? playerRow.seasonId} • ${playerRow.teamId ?? 'Team pending'}' : '${payload.seasonsById[teamRow!.seasonId]?.label ?? teamRow.seasonId} • Team season stat'; return TerminalCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w900)), const SizedBox(height: 8), Text(subtitle, style: const TextStyle(color: terminalTextSoft, height: 1.4)), const SizedBox(height: 14), Wrap(spacing: 8, runSpacing: 8, children: [InfoPill(label: playerRow == null ? 'Team stat' : 'Player stat'), InfoPill(label: (playerRow?.sourceId ?? teamRow?.sourceId) ?? 'Source pending')]), const SizedBox(height: 16), if (playerRow != null) ...[_DetailLine(label: 'Core', value: '${_fmt(playerRow.pointsPerGame)} PTS / ${_fmt(playerRow.reboundsPerGame)} REB / ${_fmt(playerRow.assistsPerGame)} AST'), _DetailLine(label: 'Defense', value: '${_fmt(playerRow.stealsPerGame)} STL / ${_fmt(playerRow.blocksPerGame)} BLK / ${_fmt(playerRow.personalFoulsPerGame)} PF'), _DetailLine(label: 'Efficiency', value: '${_pct(playerRow.trueShootingPercentage)} TS / ${_pct(playerRow.effectiveFieldGoalPercentage)} eFG / ${_pct(playerRow.usagePercentage)} USG'), _DetailLine(label: 'Ratings', value: '${_fmt(playerRow.offensiveRating)} ORtg / ${_fmt(playerRow.defensiveRating)} DRtg / ${_fmt(playerRow.netRating)} Net')], if (teamRow != null) ...[_DetailLine(label: 'Record', value: '${teamRow.wins ?? '—'} / ${teamRow.losses ?? '—'} / ${teamRow.winPercentage?.toStringAsFixed(3) ?? '—'}'), _DetailLine(label: 'Core', value: '${_fmt(teamRow.pointsPerGame)} PPG / ${_fmt(teamRow.opponentPointsPerGame)} Opp PPG / ${_fmt(teamRow.personalFoulsPerGame)} PF'), _DetailLine(label: 'Efficiency', value: '${_pct(teamRow.trueShootingPercentage)} TS / ${_pct(teamRow.effectiveFieldGoalPercentage)} eFG / ${_fmt(teamRow.pace)} Pace'), _DetailLine(label: 'Ratings', value: '${_fmt(teamRow.offensiveRating)} ORtg / ${_fmt(teamRow.defensiveRating)} DRtg / ${_fmt(teamRow.netRating)} Net')]])); } }

class _DependencyPanel extends StatelessWidget { const _DependencyPanel({required this.payload, required this.coverage}); final _StatsPayload payload; final _StatsCoverage coverage; @override Widget build(BuildContext context) => TerminalCard(padding: EdgeInsets.zero, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Padding(padding: EdgeInsets.all(18), child: Text('Stats Dependency Map', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800))), const Divider(height: 1, color: terminalBorder), DataTable(headingRowColor: WidgetStateProperty.all(terminalPanelDark), headingTextStyle: const TextStyle(color: terminalTextMuted, fontWeight: FontWeight.w700), dataTextStyle: const TextStyle(color: Color(0xFFDDE6F1)), columns: const [DataColumn(label: Text('Dependency')), DataColumn(label: Text('Ready')), DataColumn(label: Text('Use'))], rows: [_row('Player profiles', payload.players.length, 'Names and player joins'), _row('Team directory', payload.teams.length, 'Team labels and team joins'), _row('Season directory', payload.seasons.length, 'Season labels'), _row('Player joins', coverage.playerJoins, 'playerId validation'), _row('Team joins', coverage.teamJoins, 'teamId validation'), _row('Source IDs', coverage.rowsWithSource, 'Provenance')])])); DataRow _row(String label, int count, String use) => DataRow(cells: [DataCell(Text(label, style: const TextStyle(fontWeight: FontWeight.w800))), DataCell(Text('$count')), DataCell(SizedBox(width: 260, child: Text(use)))]); }
class _ChartPlanningPanel extends StatelessWidget { const _ChartPlanningPanel(); @override Widget build(BuildContext context) => const TerminalCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Future Stat Trend Charts', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800)), SizedBox(height: 10), Text('Once real rows exist, this screen should support finance-style trend charts by player, team, metric, season range, game range, regular season, playoffs, and interval. The chart layer should read from normalized local assets rather than live requests.', style: TextStyle(color: terminalTextSoft, height: 1.45))])); }
class _StatsRouteMatrix extends StatelessWidget { const _StatsRouteMatrix(); @override Widget build(BuildContext context) => TerminalCard(padding: EdgeInsets.zero, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Padding(padding: EdgeInsets.all(18), child: Text('Stats Route Matrix', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800))), const Divider(height: 1, color: terminalBorder), SingleChildScrollView(scrollDirection: Axis.horizontal, child: DataTable(headingRowColor: WidgetStateProperty.all(terminalPanelDark), headingTextStyle: const TextStyle(color: terminalTextMuted, fontWeight: FontWeight.w700), dataTextStyle: const TextStyle(color: Color(0xFFDDE6F1)), columnSpacing: 30, columns: const [DataColumn(label: Text('Route')), DataColumn(label: Text('Status')), DataColumn(label: Text('Target')), DataColumn(label: Text('Use'))], rows: [for (final item in _statRoutes) DataRow(cells: [DataCell(SizedBox(width: 180, child: Text(item.name, style: const TextStyle(fontWeight: FontWeight.w800)))), DataCell(InfoPill(label: item.status)), DataCell(SizedBox(width: 180, child: Text(item.target))), DataCell(SizedBox(width: 780, child: Text(item.use)))])]))])); }
class _MetricFamilyRegistryTable extends StatelessWidget { const _MetricFamilyRegistryTable(); @override Widget build(BuildContext context) => TerminalCard(padding: EdgeInsets.zero, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [const Padding(padding: EdgeInsets.all(18), child: Text('Metric Family Registry', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800))), const Divider(height: 1, color: terminalBorder), SingleChildScrollView(scrollDirection: Axis.horizontal, child: DataTable(headingRowColor: WidgetStateProperty.all(terminalPanelDark), headingTextStyle: const TextStyle(color: terminalTextMuted, fontWeight: FontWeight.w700), dataTextStyle: const TextStyle(color: Color(0xFFDDE6F1)), columnSpacing: 30, columns: const [DataColumn(label: Text('Family')), DataColumn(label: Text('Status')), DataColumn(label: Text('Player Fields')), DataColumn(label: Text('Team Fields')), DataColumn(label: Text('First Use')), DataColumn(label: Text('Source Note'))], rows: [for (final item in statsMetricFamilyItems) DataRow(cells: [DataCell(SizedBox(width: 170, child: Text(item.family, style: const TextStyle(fontWeight: FontWeight.w800)))), DataCell(InfoPill(label: item.status)), DataCell(SizedBox(width: 520, child: Text(item.playerFields))), DataCell(SizedBox(width: 520, child: Text(item.teamFields))), DataCell(SizedBox(width: 450, child: Text(item.firstUse))), DataCell(SizedBox(width: 420, child: Text(item.sourceNote)))])]))])); }
class _StatsCommandStageTable extends StatelessWidget { const _StatsCommandStageTable({required this.items}); final List<RegistryItem> items; @override Widget build(BuildContext context) => TerminalCard(padding: EdgeInsets.zero, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Padding(padding: const EdgeInsets.all(18), child: Row(children: [const Text('Stats Command Stage Model', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.w800)), const Spacer(), Text('${items.length} stages', style: const TextStyle(color: terminalTextMuted))])), const Divider(height: 1, color: terminalBorder), SingleChildScrollView(scrollDirection: Axis.horizontal, child: DataTable(headingRowColor: WidgetStateProperty.all(terminalPanelDark), headingTextStyle: const TextStyle(color: terminalTextMuted, fontWeight: FontWeight.w700), dataTextStyle: const TextStyle(color: Color(0xFFDDE6F1)), columnSpacing: 30, columns: const [DataColumn(label: Text('Priority')), DataColumn(label: Text('Stage')), DataColumn(label: Text('Category')), DataColumn(label: Text('Status')), DataColumn(label: Text('Description')), DataColumn(label: Text('Inputs')), DataColumn(label: Text('Next Step'))], rows: [for (final item in items) DataRow(cells: [DataCell(Text(item.priority, style: const TextStyle(fontWeight: FontWeight.w900))), DataCell(SizedBox(width: 230, child: Text(item.title, style: const TextStyle(fontWeight: FontWeight.w800)))), DataCell(SizedBox(width: 150, child: Text(item.category))), DataCell(InfoPill(label: item.status)), DataCell(SizedBox(width: 560, child: Text(item.description))), DataCell(SizedBox(width: 330, child: Text(item.inputs))), DataCell(SizedBox(width: 420, child: Text(item.nextStep)))])]))])); }
