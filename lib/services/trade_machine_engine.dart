...
class TradeAsset {
  ...
  bool get isFrozenPick =>
      _bool(metadata['frozen']) || _bool(metadata['second_apron_frozen']);
  bool get isTradeable =>
      !_bool(metadata['not_tradeable']) && !_bool(metadata['cannot_trade']);
  bool get aggregationRestricted =>
      _bool(metadata['aggregation_restricted']) ||
      _bool(metadata['recently_acquired_restricted']);
  bool get noTradeClause => _bool(metadata['no_trade_clause']);
  bool get noTradeConsent =>
      _bool(metadata['no_trade_consent']) ||
      (metadata.containsKey('trade_consent') &&
          !_bool(metadata['trade_consent']));
  bool get poisonPill => _bool(metadata['poison_pill']);
  bool get recentlySignedRestricted =>
      _bool(metadata['recently_signed_restricted']) ||
      _bool(metadata['trade_restricted']);
  bool get isUnprotectedFirst =>
      isFirstRoundPick &&
      !_bool(metadata['protected']) &&
      !_bool(metadata['swap']);
}
...
class TradeScenario {
  ...
  double outgoingSalary(String team) => outgoingFor(team)
      .where((item) => item.asset.isPlayer)
      .fold(0, (sum, item) => sum + item.asset.salary);
  double incomingSalary(String team) => incomingFor(team)
      .where((item) => item.asset.isPlayer)
      .fold(0, (sum, item) => sum + item.asset.salary);
  double postTradeSalary(String team) =>
      ...
      outgoingSalary(team) +
      incomingSalary(team);
  int outgoingPlayers(String team) =>
      outgoingFor(team).where((item) => item.asset.isPlayer).length;
  int incomingPlayers(String team) =>
      incomingFor(team).where((item) => item.asset.isPlayer).length;
  int postTradeRosterCount(String team) =>
      (capContexts[team]?.standardRosterCount ?? 15) -
      outgoingPlayers(team) +
      incomingPlayers(team);
...
class TradeValidationReport {
  ...
  bool get isValid =>
      !findings.any((item) => item.severity == TradeValidationSeverity.error);
  bool get requiresReview =>
      findings.any((item) => item.severity == TradeValidationSeverity.warning);
  ...
  int get infoCount => findings
      .where((item) => item.severity == TradeValidationSeverity.info)
      .length;
}
...
class TradeMachineEngine {
  ...
  TradeValidationReport validate(TradeScenario scenario) {
    ...
    final thresholds = NbaCbaSeasonThresholds.forSeason(
      scenario.operatingSeason,
    );
    ...
    if (scenario.teams.length > 5) {
      findings.add(
        const TradeValidationFinding(
          code: 'MAX_TEAMS',
          message:
              'Sports Terminal models a maximum of five teams in one trade scenario.',
          severity: TradeValidationSeverity.error,
        ),
      );
    }
    if (scenario.teams.toSet().length != scenario.teams.length) {
      findings.add(
        const TradeValidationFinding(
          code: 'DUPLICATE_TEAM',
          message: 'Each team may appear only once in the scenario.',
          severity: TradeValidationSeverity.error,
        ),
      );
    }
    if (thresholds == null) {
      findings.add(
        TradeValidationFinding(
          ...
          message:
              'No certified Sports Terminal CBA threshold package is installed for ${scenario.operatingSeason}; team-provided cap contexts will be used but season-level validation is incomplete.',
    ...
    for (final assignment in scenario.assignments) {
      if (!seenAssets.add(assignment.asset.id))
        duplicateAssetIds.add(assignment.asset.id);
      if (!scenario.teams.contains(assignment.asset.originTeam) ||
          !scenario.teams.contains(assignment.destinationTeam)) {
        findings.add(
          TradeValidationFinding(
            code: 'TEAM_SCOPE',
            message:
                '${assignment.asset.label} references a team outside this scenario.',
            severity: TradeValidationSeverity.error,
      ...
      if (assignment.asset.originTeam == assignment.destinationTeam) {
        findings.add(
          TradeValidationFinding(
            code: 'SAME_TEAM',
            message:
                '${assignment.asset.label} cannot be routed back to its origin team.',
            severity: TradeValidationSeverity.error,
            team: assignment.asset.originTeam,
            assetId: assignment.asset.id,
          ),
        );
      }
    ...
    for (final team in scenario.teams) {
      ...
      final allowedIncoming = context == null
          ? 0
          : _allowedIncomingSalary(context, outgoingSalary, postTradeSalary);
      ...
      summaries[team] = TeamTradeSummary(
        ...
        preTradeTier: context == null
            ? 'Unknown'
            : _salaryTier(context.teamSalary, context),
        postTradeTier: context == null
            ? 'Unknown'
            : _salaryTier(postTradeSalary, context),
      );
    ...
    if (findings.isEmpty) {
      findings.add(
        const TradeValidationFinding(
          ...
          message:
              'No modeled CBA or asset-rule failures were detected. Final league approval still depends on authoritative contracts, transaction timing and league administration.',
  ...
  double _allowedIncomingSalary(
    ...
    final exceptionStructure = firstBranch > secondBranch
        ? firstBranch
        : secondBranch;
    return roomStructure > exceptionStructure
        ? roomStructure
        : exceptionStructure;
  }
  ...
  void _validateSalaryMatching({
    ...
    if (incomingSalary > allowedIncoming + 1) {
      findings.add(
        TradeValidationFinding(
          ...
          message:
              '$team receives ${_money(incomingSalary)} but the modeled salary-matching ceiling is ${_money(allowedIncoming)}.',
          severity: TradeValidationSeverity.error,
          team: team,
      ...
      findings.add(
        TradeValidationFinding(
          ...
          message:
              '$team is ${_money(allowedIncoming - incomingSalary)} below its modeled incoming-salary ceiling.',
          severity: TradeValidationSeverity.info,
          team: team,
          ruleReference: 'CBA Article VII §6(j)',
        ),
    ...
    if (context.hardCappedAt != null &&
        postTradeSalary > context.hardCappedAt! + 1) {
      findings.add(
        TradeValidationFinding(
          code: 'HARD_CAP',
          message:
              '$team exceeds its hard cap by ${_money(postTradeSalary - context.hardCappedAt!)} after the trade.',
          severity: TradeValidationSeverity.error,
          team: team,
        ),
      );
    ...
    if (postTradeSalary > context.secondApron) {
      findings.add(
        TradeValidationFinding(
          code: 'SECOND_APRON',
          message:
              '$team is above the second apron after the trade; second-apron transaction restrictions apply.',
          severity: TradeValidationSeverity.warning,
          team: team,
        ),
      );
      ...
      findings.add(
        TradeValidationFinding(
          code: 'FIRST_APRON',
          message:
              '$team is above the first apron after the trade; first-apron salary matching and hard-cap consequences require review.',
          severity: TradeValidationSeverity.info,
          team: team,
        ),
      );
    ...
    final hasOutgoingCash = outgoing.any(
      (item) => item.asset.type == TradeAssetType.cash,
    );
    if (hasOutgoingCash && context.aboveSecondApron) {
      findings.add(
        TradeValidationFinding(
          ...
          message:
              '$team is above the second apron and is modeled as sending cash in the trade.',
          severity: TradeValidationSeverity.error,
          team: team,
    ...
    if (context.aboveSecondApron && outgoingPlayers > 1) {
      findings.add(
        TradeValidationFinding(
          ...
          message:
              '$team is above the second apron and aggregates $outgoingPlayers outgoing players for salary matching.',
          severity: TradeValidationSeverity.error,
          team: team,
    ...
    if (outgoingPlayers > 1 &&
        outgoing.any((item) => item.asset.aggregationRestricted)) {
      findings.add(
        TradeValidationFinding(
          code: 'RECENT_ACQUISITION_AGGREGATION',
          message:
              '$team aggregates a player marked as recently acquired / aggregation restricted.',
          severity: TradeValidationSeverity.error,
          team: team,
    ...
    if (usesException) {
      findings.add(
        TradeValidationFinding(
          ...
          message:
              '$team uses a trade/signing exception. Amount, expiration, aggregation and apron consequences must match the underlying exception record.',
          severity: TradeValidationSeverity.warning,
          team: team,
        ),
      );
    ...
    if (postRoster > context.maximumRosterSize) {
      findings.add(
        TradeValidationFinding(
          ...
          message:
              '$team would have $postRoster standard-roster players versus the modeled ${context.maximumRosterSize}-player regular-season maximum. A corresponding waiver/roster move may be required.',
          severity: TradeValidationSeverity.warning,
          team: team,
        ),
      );
    ...
    if (postRoster < context.minimumRosterSize) {
      findings.add(
        TradeValidationFinding(
          ...
          message:
              '$team would have $postRoster standard-roster players versus the modeled ${context.minimumRosterSize}-player minimum and may need to add players within the applicable compliance window.',
          severity: TradeValidationSeverity.warning,
          team: team,
        ),
      );
  ...
  void _validateAsset(
    ...
    if (!asset.isTradeable) {
      findings.add(
        TradeValidationFinding(
          code: 'ASSET_NOT_TRADEABLE',
          message:
              '${asset.label} is marked not tradeable in the active asset record.',
          severity: TradeValidationSeverity.error,
    ...
    if (asset.isFrozenPick) {
      findings.add(
        TradeValidationFinding(
          code: 'FROZEN_PICK',
          message:
              '${asset.label} is marked as a frozen draft pick and cannot be included without first resolving the applicable restriction.',
          severity: TradeValidationSeverity.error,
          team: asset.originTeam,
          assetId: asset.id,
          ruleReference: 'Second Apron future-pick restriction',
        ),
      );
    }
    if (asset.recentlySignedRestricted) {
      findings.add(
        TradeValidationFinding(
          code: 'PLAYER_TRADE_RESTRICTION',
          message:
              '${asset.label} is marked as subject to a signing/trade eligibility restriction.',
          severity: TradeValidationSeverity.error,
          team: asset.originTeam,
          assetId: asset.id,
        ),
    ...
    if (eligible != null && eligible.isAfter(asOf)) {
      findings.add(
        TradeValidationFinding(
          code: 'TRADE_ELIGIBILITY_DATE',
          message:
              '${asset.label} cannot be traded until ${eligible.toIso8601String().split('T').first}.',
          severity: TradeValidationSeverity.error,
          team: asset.originTeam,
          assetId: asset.id,
        ),
    ...
    if (asset.noTradeClause && !asset.noTradeConsent) {
      findings.add(
        TradeValidationFinding(
          ...
          message:
              '${asset.label} is marked with a no-trade right and no consent is recorded for this scenario.',
          severity: TradeValidationSeverity.error,
          team: asset.originTeam,
          assetId: asset.id,
        ),
    ...
    if (asset.poisonPill) {
      findings.add(
        TradeValidationFinding(
          code: 'POISON_PILL',
          message:
              '${asset.label} is marked as poison-pill affected; sending and receiving salary treatment may differ and requires contract-level calculation.',
          severity: TradeValidationSeverity.warning,
          team: asset.originTeam,
          assetId: asset.id,
        ),
    ...
    final kicker = _double(
      asset.metadata['trade_kicker_pct'] ?? asset.metadata['trade_bonus_pct'],
    );
    if (kicker > 0) {
      findings.add(
        TradeValidationFinding(
          code: 'TRADE_KICKER',
          message:
              '${asset.label} carries a ${(kicker * 100).toStringAsFixed(1)}% modeled trade bonus/kicker; confirm waiver and salary-matching treatment.',
          severity: TradeValidationSeverity.warning,
          team: asset.originTeam,
          assetId: asset.id,
        ),
  ...
  void _validatePicks(
    String team,
    List<TradeAssignment> outgoing,
    TradeScenario scenario,
    List<TradeValidationFinding> findings,
  ) {
    final picks = outgoing
        .map((item) => item.asset)
    ...
    final firstYears =
        picks
            .where(
              (asset) => asset.isUnprotectedFirst && asset.pickYear != null,
            )
            .map((asset) => asset.pickYear!)
            .toSet()
            .toList()
          ..sort();
      ...
      if (firstYears[i] - firstYears[i - 1] == 1) {
        findings.add(
          TradeValidationFinding(
            ...
            message:
                '$team sends unprotected first-round picks in consecutive future drafts (${firstYears[i - 1]} and ${firstYears[i]}). Review the Stepien Rule and the team’s remaining first-round inventory.',
            severity: TradeValidationSeverity.error,
            team: team,
            ruleReference:
                'Stepien Rule / future first-round pick restrictions',
          ),
        );
      }
    }
    for (final asset in picks) {
      if (_bool(asset.metadata['stepien_review'])) {
        findings.add(
          TradeValidationFinding(
            code: 'STEPIEN_REVIEW',
            message:
                '${asset.label} is explicitly marked for Stepien-rule review against other owned/owed first-round picks.',
            severity: TradeValidationSeverity.warning,
            team: team,
            assetId: asset.id,
          ),
      ...
      if (_bool(asset.metadata['seven_year_limit'])) {
        findings.add(
          TradeValidationFinding(
            code: 'SEVEN_YEAR_PICK_LIMIT',
            message:
                '${asset.label} is marked outside or at the edge of the permitted future-draft trading horizon.',
            severity: TradeValidationSeverity.error,
            team: team,
            assetId: asset.id,
          ),
