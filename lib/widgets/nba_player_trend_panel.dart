...
import '../services/nba_player_trend_engine.dart';
...
typedef NbaPlayerTrendGameOpenCallback = void Function(
  String gameId,
  String gameLabel,
);

class NbaPlayerTrendPanel extends StatefulWidget {
  const NbaPlayerTrendPanel({
    super.key,
    required this.seed,
  ...
  final NbaPlayerTrendGameOpenCallback onOpenGame;

  @override
  State<NbaPlayerTrendPanel> createState() => _NbaPlayerTrendPanelState();
}

class _NbaPlayerTrendPanelState extends State<NbaPlayerTrendPanel> {
  NbaPlayerTrendMetric _metric = NbaPlayerTrendMetric.points;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      key: ValueKey('player-trend-panel-${widget.playerId}'),
        ...
        children: [
          Wrap(
            ...
            children: [
              ...
              DropdownButton<NbaPlayerTrendMetric>(
                key: const ValueKey('player-trend-metric'),
                value: _metric,
                dropdownColor: _panel2,
                style: const TextStyle(color: _text, fontSize: 9),
                items: [
                  for (final metric in NbaPlayerTrendMetric.values)
              ...
              DropdownButton<int>(
                key: const ValueKey('player-trend-window'),
              ...
              _kpi('OBS', '${result.availableObservations}'),
              _kpi('AVG', _number(result.average)),
              _kpi('LAST 5', _number(result.recentFiveAverage)),
              _kpi('Δ VS PRIOR 5', _signed(result.recentDelta)),
              _pill(result.trendLabel, _trendColor(result.trendLabel)),
              if (result.missingObservations > 0)
                _pill('${result.missingObservations} GAPS', _amber),
            ],
          ),
                ...
                rows: [
                  for (final row in result.observations.reversed.take(8))
                    DataRow(cells: [
                      ...
                      DataCell(Text(row.opponentLabel.isEmpty ? '—' : row.opponentLabel)),
                      DataCell(Text(row.resultLabel)),
                      DataCell(Text(_number(row.value))),
                      DataCell(Text(_number(row.rollingAverage))),
                      DataCell(
                        TextButton(
                          key: ValueKey('player-trend-open-${row.gameId}'),
                          onPressed: row.gameId.isEmpty
                              ? null
                              : () => widget.onOpenGame(
                                    row.gameId,
                                    '${widget.playerName} · ${row.opponentLabel}',
                                  ),
          ...
          Text(
            'Dataset ${result.datasetStatus} · validation ${result.validationStatus}${result.unlinkedRows > 0 ? ' · ${result.unlinkedRows} unlinked rows excluded' : ''}',
