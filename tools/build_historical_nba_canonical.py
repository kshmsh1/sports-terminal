...
def norm_name(value: Any) -> str:
    value_text = str(value or "").lower().replace("’", "'")
    value_text = re.sub(r"\b(jr|sr|ii|iii|iv)\.?\b", "", value_text)
...
def infer_league(row: dict[str, Any], source_table: str, fallback: str = "NBA") -> str:
    ...
    return raw if raw in {"NBA", "ABA", "BAA"} else fallback
...
def initialize_schema(db: sqlite3.Connection) -> None:
    db.executescript(
        """
        CREATE TABLE canon_build_manifest(build_id TEXT PRIMARY KEY,schema_version TEXT NOT NULL,built_at TEXT NOT NULL,source_rows INTEGER NOT NULL,source_tables INTEGER NOT NULL,source_count INTEGER NOT NULL,canonical_counts_json TEXT NOT NULL,warnings_json TEXT NOT NULL);
        ...
        CREATE TABLE canon_dim_team(team_key TEXT PRIMARY KEY,franchise_key TEXT,canonical_name TEXT NOT NULL,abbreviation TEXT,league_id TEXT,active_from TEXT,active_to TEXT,nba_team_id TEXT,source_count INTEGER NOT NULL DEFAULT 1,provenance_json TEXT NOT NULL DEFAULT '{}');
        CREATE TABLE canon_team_source_xref(source_key TEXT NOT NULL,source_table TEXT NOT NULL,source_id TEXT NOT NULL DEFAULT '',source_name TEXT NOT NULL DEFAULT '',source_abbreviation TEXT NOT NULL DEFAULT '',team_key TEXT NOT NULL,match_method TEXT NOT NULL,confidence REAL NOT NULL,evidence_json TEXT NOT NULL DEFAULT '{}',PRIMARY KEY(source_key,source_table,source_id,source_name,source_abbreviation));
        CREATE TABLE canon_dim_player(player_key TEXT PRIMARY KEY,canonical_name TEXT NOT NULL,normalized_name TEXT NOT NULL,nba_id TEXT,bref_id TEXT,birth_date TEXT,birth_year INTEGER,primary_position TEXT,active_from TEXT,active_to TEXT,source_count INTEGER NOT NULL,identity_confidence REAL NOT NULL,provenance_json TEXT NOT NULL DEFAULT '{}');
        CREATE TABLE canon_player_source_xref(source_key TEXT NOT NULL,source_table TEXT NOT NULL,source_id TEXT NOT NULL DEFAULT '',source_name TEXT NOT NULL DEFAULT '',player_key TEXT NOT NULL,match_method TEXT NOT NULL,confidence REAL NOT NULL,evidence_json TEXT NOT NULL DEFAULT '{}',PRIMARY KEY(source_key,source_table,source_id,source_name));
        CREATE INDEX idx_canon_player_xref_lookup ON canon_player_source_xref(source_key,source_id,confidence DESC);
        CREATE TABLE canon_dim_game(game_key TEXT PRIMARY KEY,nba_game_id TEXT,game_date TEXT,season_id TEXT,league_id TEXT,season_type TEXT NOT NULL,home_team_key TEXT,away_team_key TEXT,home_score REAL,away_score REAL,winner_team_key TEXT,status TEXT,source_count INTEGER NOT NULL,provenance_json TEXT NOT NULL DEFAULT '{}');
        CREATE TABLE canon_game_source_xref(source_key TEXT NOT NULL,source_table TEXT NOT NULL,source_id TEXT NOT NULL DEFAULT '',game_key TEXT NOT NULL,match_method TEXT NOT NULL,confidence REAL NOT NULL,evidence_json TEXT NOT NULL DEFAULT '{}',PRIMARY KEY(source_key,source_table,source_id));
        CREATE TABLE canon_fact_team_game(game_key TEXT NOT NULL,team_key TEXT NOT NULL,opponent_team_key TEXT,is_home INTEGER NOT NULL,result TEXT,points REAL,opponent_points REAL,fgm REAL,fga REAL,three_pm REAL,three_pa REAL,ftm REAL,fta REAL,orb REAL,drb REAL,reb REAL,ast REAL,stl REAL,blk REAL,tov REAL,pf REAL,source_key TEXT NOT NULL,provenance_json TEXT NOT NULL DEFAULT '{}',PRIMARY KEY(game_key,team_key));
        CREATE TABLE canon_fact_player_season(fact_key TEXT PRIMARY KEY,player_key TEXT NOT NULL,season_id TEXT NOT NULL,league_id TEXT NOT NULL,season_type TEXT NOT NULL,team_key TEXT,team_abbreviation TEXT,position TEXT,age REAL,games REAL,games_started REAL,minutes REAL,fgm REAL,fga REAL,fg_pct REAL,three_pm REAL,three_pa REAL,three_pct REAL,two_pm REAL,two_pa REAL,two_pct REAL,ftm REAL,fta REAL,ft_pct REAL,orb REAL,drb REAL,reb REAL,ast REAL,stl REAL,blk REAL,tov REAL,pf REAL,pts REAL,per REAL,ts_pct REAL,efg_pct REAL,ws REAL,ws48 REAL,obpm REAL,dbpm REAL,bpm REAL,vorp REAL,usg_pct REAL,ortg REAL,drtg REAL,primary_source TEXT NOT NULL,source_count INTEGER NOT NULL,provenance_json TEXT NOT NULL);
        ...
        CREATE TABLE canon_fact_team_season(fact_key TEXT PRIMARY KEY,team_key TEXT,team_abbreviation TEXT,team_name TEXT,season_id TEXT NOT NULL,league_id TEXT NOT NULL,season_type TEXT NOT NULL,games REAL,wins REAL,losses REAL,minutes REAL,pts REAL,opp_pts REAL,fgm REAL,fga REAL,three_pm REAL,three_pa REAL,ftm REAL,fta REAL,orb REAL,drb REAL,reb REAL,ast REAL,stl REAL,blk REAL,tov REAL,pf REAL,pace REAL,ortg REAL,drtg REAL,net_rtg REAL,srs REAL,primary_source TEXT NOT NULL,source_count INTEGER NOT NULL,provenance_json TEXT NOT NULL);
        CREATE INDEX idx_canon_team_season_lookup ON canon_fact_team_season(season_id,league_id);
        CREATE TABLE canon_fact_player_game(fact_key TEXT PRIMARY KEY,game_key TEXT,source_game_id TEXT,player_key TEXT,player_name TEXT NOT NULL,team_key TEXT,team_abbreviation TEXT,opponent_team_key TEXT,opponent_abbreviation TEXT,season_id TEXT NOT NULL,league_id TEXT NOT NULL,season_type TEXT NOT NULL,game_date TEXT,minutes REAL,pts REAL,reb REAL,ast REAL,stl REAL,blk REAL,tov REAL,pf REAL,ts_pct REAL,efg_pct REAL,usg_pct REAL,ortg REAL,drtg REAL,bpm REAL,source_key TEXT NOT NULL,source_table TEXT NOT NULL,source_row INTEGER,provenance_json TEXT NOT NULL DEFAULT '{}');
        ...
        CREATE TABLE canon_fact_award(award_key TEXT PRIMARY KEY,player_key TEXT,player_name TEXT,season_id TEXT,league_id TEXT,award TEXT NOT NULL,rank_text TEXT,winner INTEGER,share REAL,source_key TEXT NOT NULL,payload_json TEXT NOT NULL);
        CREATE TABLE canon_fact_all_star(selection_key TEXT PRIMARY KEY,player_key TEXT,player_name TEXT,season_id TEXT,league_id TEXT,team_text TEXT,source_key TEXT NOT NULL,payload_json TEXT NOT NULL);
        CREATE TABLE canon_fact_draft(draft_key TEXT PRIMARY KEY,player_key TEXT,player_name TEXT,draft_year INTEGER,league_id TEXT,round_text TEXT,pick_number REAL,drafting_team_key TEXT,drafting_team_text TEXT,source_key TEXT NOT NULL,payload_json TEXT NOT NULL);
        CREATE TABLE canon_conflicts(conflict_id INTEGER PRIMARY KEY AUTOINCREMENT,entity_type TEXT NOT NULL,entity_key TEXT NOT NULL,field_name TEXT NOT NULL,selected_source TEXT NOT NULL,selected_value TEXT,alternate_source TEXT NOT NULL,alternate_value TEXT,severity TEXT NOT NULL,detected_at TEXT NOT NULL);
        CREATE INDEX idx_canon_conflicts_entity ON canon_conflicts(entity_type,entity_key);
        CREATE TABLE canon_field_provenance(entity_type TEXT NOT NULL,entity_key TEXT NOT NULL,field_name TEXT NOT NULL,source_key TEXT NOT NULL,source_table TEXT,source_row TEXT,source_value TEXT,selected INTEGER NOT NULL,evidence_json TEXT NOT NULL DEFAULT '{}',PRIMARY KEY(entity_type,entity_key,field_name,source_key,source_table,source_row));
        CREATE INDEX idx_canon_provenance_entity ON canon_field_provenance(entity_type,entity_key);
        CREATE TABLE canon_coverage(domain TEXT NOT NULL,league_id TEXT,season_id TEXT,row_count INTEGER NOT NULL,source_count INTEGER NOT NULL,sources_json TEXT NOT NULL,PRIMARY KEY(domain,league_id,season_id));
        """
    )
...
def repair_inventory(db: sqlite3.Connection) -> int:
    ...
    for source_key, source_table, warehouse_table, columns_json in inventory:
        ...
        if str(source_key) == "gonzalo_all_time" and match:
            domain, grain = ("player_game", "player_game") if match.group("kind").lower() == "advanced" else ("historical_game_detail", "source_native_game_detail")
            ...
            domain, grain = "player_season", "player_season"
        if domain:
            db.execute("UPDATE historical_table_inventory SET domain=?,grain=? WHERE source_key=? AND source_table=?", (domain, grain, source_key, source_table))
            repaired += 1
        try:
        ...
        if season_candidates:
            chosen = season_candidates[0]
            values = db.execute(f"SELECT DISTINCT {q(chosen)} FROM {q(str(warehouse_table))} WHERE {q(chosen)} IS NOT NULL LIMIT 2000").fetchall()
            normalized = sorted(value for value in (normalize_season(row[0], source_key=str(source_key), source_table=source_name)[0] for row in values) if value)
            if normalized:
                db.execute("UPDATE historical_table_inventory SET season_column=?,min_season=?,max_season=? WHERE source_key=? AND source_table=?", (chosen, normalized[0], normalized[-1], source_key, source_table))
    db.commit()
    return repaired
...
def ensure_season(db: sqlite3.Connection, season_id: str | None, start: int | None, end: int | None) -> None:
    if season_id and start is not None and end is not None:
...
def extract_player_fact(row: dict[str, Any], source_key: str, source_table: str, player_ids: dict[tuple[str, str], str], player_names: dict[str, str], team_ids: dict[tuple[str, str], str], team_abbrs: dict[str, str]) -> tuple[str, dict[str, Any]] | None:
    ...
    if not player_key and player_name:
        player_key = f"unresolved_{hashlib.sha1(norm_name(player_name).encode()).hexdigest()[:14]}"
    if not player_key:
        return None
...
def build_player_seasons(db: sqlite3.Connection, policy: dict[str, Any], player_ids: dict[tuple[str, str], str], player_names: dict[str, str], team_ids: dict[tuple[str, str], str], team_abbrs: dict[str, str]) -> int:
    ...
    for source_key, source_table in [("sumitro_bref_history", "Player Totals"), ("sumitro_bref_history", "Advanced"), ("sumitro_bref_history", "Player Season Info"), ("gonzalo_all_time", "totals_stats"), ("gonzalo_all_time", "advanced_stats")]:
        ...
        for row in rows(db, table):
            ...
            start, end = payload.pop("start_year"), payload.pop("end_year")
...
def build_games(db: sqlite3.Connection, policy: dict[str, Any], team_ids: dict[tuple[str, str], str], team_abbrs: dict[str, str]) -> tuple[int, int]:
    ...
    for row in rows(db, game_table):
        ...
        db.execute("INSERT OR REPLACE INTO canon_dim_game VALUES (?,?,?,?,?,?,?,?,?,?,?,?,1,?)", (game_key, game_id, game_date or None, season_id, league, season_type, home_key, away_key, home_score, away_score, winner, text(first(row, ("game_status_text", "status"))) or None, json.dumps({"game": "wyatt_nbadb", "score": "wyatt_nbadb"})))
        db.execute("INSERT OR REPLACE INTO canon_game_source_xref VALUES ('wyatt_nbadb','game',?,?,'nba_game_id',1.0,'{}')", (game_id, game_key))
        game_count += 1
        for side, team_key, opponent_key, points, opponent_points in [("home", home_key, away_key, home_score, away_score), ("away", away_key, home_key, away_score, home_score)]:
            if not team_key:
                continue
            metrics = {field: number(home_away_value(row, side, aliases)) for field, aliases in TEAM_METRICS.items() if field not in {"games", "wins", "losses", "pace", "ortg", "drtg", "net_rtg", "srs"}}
            result = "W" if points is not None and opponent_points is not None and points > opponent_points else ("L" if points is not None and opponent_points is not None and points < opponent_points else None)
            db.execute("INSERT OR REPLACE INTO canon_fact_team_game(game_key,team_key,opponent_team_key,is_home,result,points,opponent_points,fgm,fga,three_pm,three_pa,ftm,fta,orb,drb,reb,ast,stl,blk,tov,pf,source_key,provenance_json) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (game_key, team_key, opponent_key, 1 if side == "home" else 0, result, points, opponent_points, metrics.get("fgm"), metrics.get("fga"), metrics.get("three_pm"), metrics.get("three_pa"), metrics.get("ftm"), metrics.get("fta"), metrics.get("orb"), metrics.get("drb"), metrics.get("reb"), metrics.get("ast"), metrics.get("stl"), metrics.get("blk"), metrics.get("tov"), metrics.get("pf"), "wyatt_nbadb", json.dumps({"all": "wyatt_nbadb"})))
            team_game_count += 1
    db.commit()
...
def create_play_by_play_view(db: sqlite3.Connection) -> bool:
    ...
    def expr(aliases: Iterable[str], default: str = "NULL") -> str:
        for alias in aliases:
            if safe(alias) in columns:
                return f"p.{q(columns[safe(alias)])}"
        return default
    ...
    # canon_player_source_xref intentionally preserves multiple evidence records per
    # source ID. Collapse it before the 13M+ event join so evidence multiplicity can
    # never fan a single source event out into duplicate canonical events.
    player_projection = """
      (SELECT source_id, MIN(player_key) AS player_key
       FROM canon_player_source_xref
       WHERE source_key='wyatt_nbadb' AND source_id<>''
       GROUP BY source_id)
    """
    db.execute(
        f"""
        ...
        SELECT gx.game_key,
               ...
               {period} AS period,
               {clock} AS clock,
               TRIM(COALESCE(CAST({home_desc} AS TEXT),'') ||
                    CASE WHEN COALESCE(CAST({neutral_desc} AS TEXT),'')<>'' THEN ' | '||CAST({neutral_desc} AS TEXT) ELSE '' END ||
                    CASE WHEN COALESCE(CAST({visitor_desc} AS TEXT),'')<>'' THEN ' | '||CAST({visitor_desc} AS TEXT) ELSE '' END) AS description,
               {score} AS score,
        ...
        FROM {q(table)} p
        LEFT JOIN canon_game_source_xref gx
          ON gx.source_key='wyatt_nbadb' AND gx.source_table='game' AND gx.source_id=CAST({game_id} AS TEXT)
        LEFT JOIN {player_projection} p1 ON p1.source_id=CAST({player1} AS TEXT)
        LEFT JOIN {player_projection} p2 ON p2.source_id=CAST({player2} AS TEXT)
        LEFT JOIN {player_projection} p3 ON p3.source_id=CAST({player3} AS TEXT)
        """
    )
    return True
...
def build_events_awards_draft(db: sqlite3.Connection, player_ids: dict[tuple[str, str], str], player_names: dict[str, str], team_ids: dict[tuple[str, str], str], team_abbrs: dict[str, str]) -> tuple[int, int, int]:
    ...
    for source_table in ("Player Award Shares", "End of Season Teams", "End of Season Teams (Voting)"):
        ...
        for row in rows(db, table):
            ...
            db.execute("INSERT OR REPLACE INTO canon_fact_award VALUES (?,?,?,?,?,?,?,?,?,?,?)", (f"award_{key}", player_key, player_name or None, season_id, league, award, rank_text, winner, share, "sumitro_bref_history", payload))
            award_count += 1
    ...
    if table:
        for row in rows(db, table):
            ...
            db.execute("INSERT OR REPLACE INTO canon_fact_all_star VALUES (?,?,?,?,?,?,?,?)", (f"allstar_{key}", player_key, player_name or None, season_id, league, text(first(row, ("team", "selection", "type"))) or None, "sumitro_bref_history", payload))
            all_star_count += 1
    ...
    if table:
        for row in rows(db, table):
            ...
            db.execute("INSERT OR REPLACE INTO canon_fact_draft VALUES (?,?,?,?,?,?,?,?,?,?,?)", (f"draft_{key}", player_key, player_name or None, draft_year, league, round_text, pick_number, team_abbrs.get(team_text), team_text or None, "sumitro_bref_history", payload))
            draft_count += 1
    db.commit()
...
def build_coverage(db: sqlite3.Connection) -> int:
    ...
    for domain, table, league_col, season_col, source_col in domains:
        ...
        for league, season, count, source_count, source_list in db.execute(query):
            ...
            db.execute("INSERT OR REPLACE INTO canon_coverage VALUES (?,?,?,?,?,?)", (domain, league, season, int(count), int(source_count), json.dumps([source for source in sources if source])))
            inserted += 1
    db.commit()
    return inserted
...
def update_league_bounds(db: sqlite3.Connection) -> None:
    for league_id, league_name in (("NBA", "National Basketball Association"), ("ABA", "American Basketball Association"), ("BAA", "Basketball Association of America")):
        row = db.execute("SELECT MIN(season_id),MAX(season_id) FROM (SELECT season_id FROM canon_fact_player_season WHERE league_id=? UNION ALL SELECT season_id FROM canon_dim_game WHERE league_id=?)", (league_id, league_id)).fetchone()
...
def main() -> int:
    ...
    report: dict[str, Any] = {"status": "running", "startedAt": now_iso(), "database": str(database), "policy": str(policy_path)}
    try:
        reset_canonical(db)
