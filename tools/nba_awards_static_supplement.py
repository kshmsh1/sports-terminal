...
def _load_full_user_rows() -> list[dict[str, Any]]:
    ...
    spec = importlib.util.spec_from_file_location(
        "nba_awards_user_supplement_full_runtime", path
    )
...
def _load_external_curated_rows() -> list[dict[str, Any]]:
    """Load an optional normalized curated awards file.

    Large team-honor histories such as All-NBA, All-Defense and All-Rookie are
    safer to ingest as structured rows than to infer from PDF text at build
    time. `tools/import_curated_nba_awards.py` writes this contract.
    """
    candidates = (
        ROOT / "raw" / "curated" / "nba_awards.json",
        ROOT.parent / "raw" / "curated" / "nba_awards.json",
    )
    for path in candidates:
        ...
        for item in rows:
            if not isinstance(item, dict):
                continue
            row = dict(item)
            row.setdefault("league_id", "NBA")
            row.setdefault("source_key", "curated_nba_awards_import")
            row.setdefault("manual_static_supplement", True)
            row.setdefault("player_honor", True)
            result.append(row)
        return result
    return []
...
def _recent_award_rows() -> list[dict[str, Any]]:
    ...
    rows.extend(
        ...
        for season_id, key, label, player, team, player_honor in annual
    )
    return rows
...
def supplemental_awards() -> list[dict[str, Any]]:
    return [
        *_asg_mvp_rows(),
        *_recent_award_rows(),
        *_load_full_user_rows(),
        *_load_external_curated_rows(),
    ]
...
def apply_award_supplement(output: Path) -> dict[str, int]:
    ...
    history.sort(
        key=lambda row: (
            str(row.get("season_id") or ""),
            str(row.get("award_key") or row.get("award") or ""),
    ...
    if isinstance(manifest, dict):
        external_rows = _load_external_curated_rows()
        manifest["award_count"] = len(history)
        manifest["manual_awards_supplement"] = {
            "source_key": SOURCE_KEY,
            "rows": len(additions),
            "external_curated_rows": len(external_rows),
            "added_history_rows": added_history,
            "added_player_honors": added_player,
            "unmatched_player_names": sorted(
                name for name in unmatched if name
            ),
        }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Merge curated static NBA awards into a compiled corpus."
    )
    parser.add_argument(
        "--output",
    ...
    print(
        f"Static NBA awards supplement: {result['added_history']} history rows; "
        f"{result['added_player']} player-honor rows; "
