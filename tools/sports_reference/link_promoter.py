    ...
    def promote(
        ...
        *,
        ...
        clauses = [
            ...
            f"link.page_family IN ({placeholders})",
        ]
        ...
        query = f"""
            SELECT
              ...
              link.season_end_year AS season_end_year,
            ...
            GROUP BY link.target_url, link.page_family, link.source_key,
                     link.season_end_year, link.team_abbreviation
        ...
        with self.store.connect() as db:
            ...
            existing_urls = {
                ...
                for row in db.execute(
                    "SELECT url FROM pages WHERE url IN (SELECT target_url FROM discovered_links)"
                )
            }
        ...
        existing_count = sum(1 for row in selected if row["url"] in existing_urls)
        inserted_count = 0
