...
# Compile the roster parser, joins, completeness analysis, and validator before mutating connected assets.
flutter test \
...
backup_dir="$(mktemp -d)"
manual_report_existed=0
completeness_report_existed=0
...
if [[ -f raw/manual_roster_seed_report.json ]]; then
  ...
  manual_report_existed=1
fi
if [[ -f raw/roster_completeness_report.json ]]; then
  cp raw/roster_completeness_report.json "$backup_dir/roster_completeness_report.json"
  completeness_report_existed=1
fi
...
restore_on_failure() {
  ...
  if [[ $status -ne 0 ]]; then
    ...
    if [[ $manual_report_existed -eq 1 ]]; then
      cp "$backup_dir/manual_roster_seed_report.json" raw/manual_roster_seed_report.json
    else
      rm -f raw/manual_roster_seed_report.json
    fi
    if [[ $completeness_report_existed -eq 1 ]]; then
      cp "$backup_dir/roster_completeness_report.json" raw/roster_completeness_report.json
    else
      rm -f raw/roster_completeness_report.json
    fi
    echo "Manual roster seed failed. Connected roster assets and reports were restored from backup."
  fi
  rm -rf "$backup_dir"
  exit "$status"
}
