...
def _surface(
    ...
    *,
    ...
    return NbaStatsSurface(
        ...
        measure_type_hint=measure_type_hint,
...
SURFACES: dict[str, NbaStatsSurface] = {
    "players_advanced": _surface(
        "players_advanced", "Players / General / Advanced", "player", "advanced",
        "/stats/players/advanced", "player-season filtered aggregate", "1996-97",
        endpoint_hint="leaguedashplayerstats", measure_type_hint="Advanced",
    ...
    "lineups_advanced": _surface("lineups_advanced", "Lineups / General / Advanced", "lineup", "lineups", "/stats/lineups/advanced", "lineup-season filtered aggregate", "2008-09", notes=("NBA FAQ says lineup data goes back to 2008; exact first season should be confirmed during discovery.",)),
    "players_advanced_box_scores": _surface(
        "players_advanced_box_scores", "Players / Advanced Box Scores / Advanced", "player", "advanced_box_score",
        "/stats/players/boxscores-advanced", "player-game", "1996-97",
        endpoint_hint="playergamelogs", measure_type_hint="Advanced",
        ...
        notes=(
            ...
            "Chrome capture on 2026-08-16 confirmed GET stats.nba.com/stats/playergamelogs with MeasureType=Advanced and PerMode=Totals.",
            "Response resource is gamelogs and result set is PlayerGameLogs.",
        ),
    ),
    "teams_advanced_box_scores": _surface(
        "teams_advanced_box_scores", "Teams / Advanced Box Scores / Advanced", "team", "advanced_box_score",
        "/stats/teams/boxscores-advanced", "team-game", "1996-97",
        endpoint_hint="teamgamelogs", measure_type_hint="Advanced",
        discovery_status="confirmed_browser_capture_schema_confirmed",
        notes=(
            "Chrome capture on 2026-08-16 confirmed GET stats.nba.com/stats/teamgamelogs with MeasureType=Advanced and PerMode=Totals.",
            "Response resource is gamelogs and result set is TeamGameLogs.",
            "Confirmed columns include team/game identity, offensive/defensive/net ratings, assist and rebound rates, turnover rate, eFG%, TS%, pace, possessions, PIE, and rank/availability metadata.",
        ),
    ),
...
def surface_for_referer(referer: str | None) -> NbaStatsSurface | None:
    ...
    if not matches:
        return None
    return max(matches, key=lambda surface: len(surface.page_path))


def registry_payload() -> dict[str, object]:
    return {
        "contract": "sports-terminal-nba-com-stats-surface-registry-v1",
        "official_coverage": {
...
def main() -> int:
    ...
    if args.surface:
        print(json.dumps(SURFACES[args.surface].to_dict(), indent=2))
        return 0
    if args.json:
