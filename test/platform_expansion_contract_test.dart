...
void main() {
  test('platform shell owns vertical page scrolling', () {
    final shell = File('lib/widgets/connected_role_terminal_shell.dart').readAsStringSync();
  ...
  test('basic Stats is basic-only and links every rendered player/team', () {
    final stats = File('lib/screens/product_nba_basic_stats_screen.dart').readAsStringSync();
  ...
  test('NBA Hub exposes league-wide product desks and linked entities', () {
    final hub = File('lib/screens/product_nba_hub_v2_screen.dart').readAsStringSync();
  ...
  test('account creation requires separately versioned Terms and Privacy acceptance', () {
    final login = File('lib/screens/login_screen.dart').readAsStringSync();
    final authClient = File('lib/services/launch_auth_client.dart').readAsStringSync();
  ...
  test('Python output is structurally below code rather than a desktop side pane', () {
    final python = File('lib/screens/product_python_lab_v2_screen.dart').readAsStringSync();
    ...
    expect(python, contains('Output intentionally sits below the notebook code cell at every width.'));
  });

  test('team blogs community articles and profile are substantive product surfaces', () {
    final blogs = File('lib/screens/product_team_blogs_screen.dart').readAsStringSync();
    final community = File('lib/screens/product_community_v2_screen.dart').readAsStringSync();
    final articles = File('lib/screens/product_articles_v2_screen.dart').readAsStringSync();
    final profile = File('lib/screens/product_profile_persisted_screen.dart').readAsStringSync();

    for (final label in const ['Coverage', 'Roster', 'Schedule', 'Stats', 'Fan Room']) {
      expect(blogs, contains(label));
    }
    for (final label in const ['Hot', 'New', 'Top', 'Most Discussed', 'Rising']) {
      expect(community, contains(label));
    }
    for (final sport in const ['NBA', 'WNBA', 'NFL', 'NHL', 'MLB', 'NCAAM', 'NCAAW', 'College Football', 'Tennis', 'MLS', 'Premier League', 'Champions League']) {
      expect(articles, contains("'$sport'"));
    }
    for (final label in const ['Identity', 'Teams', 'Interests', 'Notifications', 'Privacy', 'Awards & Badges']) {
